...
#define EM_NOOC_TARGET EM_AARCH64
        ...
        case R_AARCH64_ABS32:
            if (s1->output_type & NOOC_OUTPUT_DYN) {
                /* XXX: this logic may depend on NOOC's codegen
                   now NOOC uses R_AARCH64_RELATIVE even for a 64bit pointer */
                qrel->r_offset = rel->r_offset;
                qrel->r_info = ELFW(R_INFO)(0, R_AARCH64_RELATIVE);
                /* Use sign extension! */
	...
	case R_AARCH64_PREL32:
        ...
        case R_AARCH64_ADR_PREL_PG_HI21: {
            ...
            if ((off + ((uint64_t)1 << 20)) >> 21)
                nooc_error_noabort("R_AARCH64_ADR_PREL_PG_HI21 relocation failed");
	    ...
	    printf ("reloc %d @ 0x%lx: val=0x%lx name=%s\n", type, addr, val,
            ...
            if (((val - addr) + ((uint64_t)1 << 27)) & ~(uint64_t)0xffffffc)
                nooc_error_noabort("R_AARCH64_(JUMP|CALL)26 relocation failed"
        ...
        case R_AARCH64_ADR_GOT_PAGE: {
            ...
            if ((off + ((uint64_t)1 << 20)) >> 21)
                nooc_error_noabort("R_AARCH64_ADR_GOT_PAGE relocation failed");
            write32le(ptr, ((read32le(ptr) & 0x9f00001f) |
                            (off & 0x1ffffc) << 3 | (off & 3) << 29));
            return;
        }
