...
ST_FUNC void noocelf_delete(NOOCState *s1)
{
    int i;
    ...
    for (i = 0; i < nb_sym_versions; i++) {
        nooc_free(sym_versions[i].version);
        nooc_free(sym_versions[i].lib);
    }
    nooc_free(sym_versions);
    nooc_free(sym_to_version);
#endif
...
# endif
    ...
    nooc_free(s1->sym_attrs);

    symtab_section = NULL; /* for noocrun.c:rt_printline() */
}
...
ST_FUNC void noocelf_end_file(NOOCState *s1)
{
    ...
    tr = nooc_mallocz(nb_syms * sizeof *tr);
...
ST_FUNC Section *new_section(NOOCState *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    sec = nooc_mallocz(sizeof(Section) + strlen(name));
    sec->s1 = s1;
...
ST_FUNC Section *new_symtab(NOOCState *s1,
...
ST_FUNC void section_realloc(Section *sec, unsigned long new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    data = nooc_realloc(sec->data, size);
...
static Section *have_section(NOOCState *s1, const char *name)
{
    Section *sec;
    int i;
    for(i = 1; i < s1->nb_sections; i++) {
...
/* return elf symbol value, signal error if 'err' is nonzero, decorate
   name if FORC */
ST_FUNC addr_t get_sym_addr(NOOCState *s1, const char *name, int err, int forc)
{
    int sym_index;
    ElfW(Sym) *sym;
    char buf[256];
    ...
    if (!sym_index || sym->st_shndx == SHN_UNDEF) {
        if (err)
            nooc_error_noabort("%s not defined", name);
        return (addr_t)-1;
    }
    return sym->st_value;
}
...
version_add (NOOCState *s1)
{
    ...
    if (nb_versions > 2) {
        ...
        for (i = nb_sym_versions; i-- > 0;) {
            ...
            /* abitest-nooc fails on older i386-linux with "ld-linux.so.2" DT_NEEDED
...
#endif
                nooc_error_noabort("'%s' defined twice", name);
            }
        } else {
            esym->st_other = other;
        do_patch:
                            ...
                            int type, int symbol, addr_t addend)
{
    ...
    if (SHT_RELX != SHT_RELA && addend)
        nooc_error_noabort("non-zero addend on REL architecture");
}
...
ST_FUNC struct sym_attr *get_sym_attr(NOOCState *s1, int index, int alloc)
{
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        while (index >= n)
            n *= 2;
        tab = nooc_realloc(s1->sym_attrs, n * sizeof(*s1->sym_attrs));
        s1->sym_attrs = tab;
...
static void modify_reloctions_old_to_new(NOOCState *s1, Section *s, int *old_to_new_syms)
{
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since NOOC cannot sort it while generating
   the code, we must do it after. All the relocation tables are also
   modified to take into account the symbol table sorting */
static void sort_syms(NOOCState *s1, Section *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = nooc_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = nooc_malloc(nb_syms * sizeof(int));
...
static Section *create_gnu_hash(NOOCState *s1)
{
...
static void update_gnu_hash(NOOCState *s1, Section *gnu_hash)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    ...
    new_syms = nooc_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = nooc_malloc(nb_syms * sizeof(int));
    hash = nooc_malloc(nb_syms * sizeof(Elf32_Word));
    nextbuck = nooc_malloc(nb_syms * sizeof(int));
    ...
    buck = nooc_malloc(nbuckets * sizeof(*buck));

    if (gnu_hash->data_offset != 4 * 4 +
				 PTR_SIZE * bloom_size +
				 nbuckets * 4 +
				 (nb_syms - (q - new_syms)) * 4)
	nooc_error_noabort ("gnu_hash size incorrect");
	...
	if (1/*versym*/) {
            newver = nooc_malloc(nb_syms * sizeof(*newver));
	    for (i = 0; i < nb_syms; i++)
...
ST_FUNC void relocate_syms(NOOCState *s1, Section *symtab, int do_resolve)
{
    ...
    for_each_elem(symtab, 1, sym, ElfW(Sym)) {
        ...
        if (sh_num == SHN_UNDEF) {
            ...
            /* Use ld.so to resolve symbol for us (for nooc -run) */
            if (do_resolve) {
#if defined NOOC_IS_NATIVE && !defined NOOC_TARGET_PE
		...
		if (addr == NULL) {
            ...
            if (sym_bind == STB_WEAK)
                ...
                nooc_error_noabort("undefined symbol '%s'", name);
...
static void relocate_section(NOOCState *s1, Section *s, Section *sr)
{
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
    int type, sym_index;
    ...
    if (sr->sh_flags & SHF_ALLOC) {
        ...
        if (s1->output_type & NOOC_OUTPUT_DYN) {
            ...
            if (r && 0 == (s->sh_flags & SHF_WRITE))
                nooc_warning("%d relocations to ro-section %s", (unsigned)(r / sizeof *qrel), s->name);
#endif
        }
    }
#endif
...
#ifndef NOOC_TARGET_MACHO
        if (s != s1->got
            || s1->static_link
            || s1->output_type == NOOC_OUTPUT_MEMORY)
#endif
        {
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int prepare_dynamic_rel(NOOCState *s1, Section *sr)
{
    int count = 0;
#if defined(NOOC_TARGET_I386) || defined(NOOC_TARGET_X86_64) || \
    defined(NOOC_TARGET_ARM) || defined(NOOC_TARGET_ARM64) || \
    defined(NOOC_TARGET_RISCV64)
    ElfW_Rel *rel;
    for_each_elem(sr, 0, rel, ElfW_Rel) {
...
static int build_got(NOOCState *s1)
{
...
static struct sym_attr * put_got_entry(NOOCState *s1, int dyn_reloc_type,
                                       int sym_index)
{
    int need_plt_entry;
    const char *name;
...
ST_FUNC void build_got_entries(NOOCState *s1, int got_sym)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
...
redo:
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
            ...
            if (gotplt_entry == -1) {
                nooc_error_noabort ("Unknown relocation type for got: %d", type);
                continue;
            }
            sym_index = ELFW(R_SYM)(rel->r_info);
            sym = &((ElfW(Sym) *)symtab_section->data)[sym_index];
	       ...
	       probably created by nooc_add_symbol, and thus on 64-bit
	       targets might be too far from application code.  */
		    ...
		    int dynindex;
                    if (!PCRELATIVE_DLLPLT
                        && (s1->output_type & NOOC_OUTPUT_DYN))
                        continue;
		    /* Relocations for UNDEF symbols would normally need
		       ...
		       But NOOC doesn't do that (at least for exes), so we
			...
			if (dynindex
			    ...
			    goto jmp_slot;
                    ...
                    if (sym->st_value == 0) /* from nooc_add_btstub() */
                        continue;
#ifndef NOOC_TARGET_ARM
                    if (PTR_SIZE != 8)
                        continue;
#endif
                    /* from nooc_add_symbol(): on 64 bit platforms these
		...
		sym->st_shndx != SHN_UNDEF &&
		 ...
		 ELFW(ST_BIND)(sym->st_info) == STB_LOCAL ||
		 s1->output_type & NOOC_OUTPUT_EXE)) {
		if (pass != 0)
		    continue;
            ...
            if (reloc_type == -1) {
                nooc_error_noabort ("Unknown relocation type: %d", type);
                continue;
            }
...
static void add_init_array_defines(NOOCState *s1, const char *section_name)
{
    Section *s;
    addr_t end_offset;
    char buf[1024];
...
static void set_local_sym(NOOCState *s1, const char *name, Section *s, int offset)
{
...
static void nooc_compile_string_no_debug(NOOCState *s, const char *str)
{
    ...
    s->do_debug = 0;
    s->test_coverage = 0;
    nooc_compile_string(s, str);
    s->do_debug = save_do_debug;
    s->test_coverage = save_test_coverage;
}

#ifdef CONFIG_NOOC_BACKTRACE
static void put_ptr(NOOCState *s1, Section *s, int offs)
{
    int c;
...
ST_FUNC void nooc_add_btstub(NOOCState *s1)
{
    ...
    o = s->data_offset;
    /* create (part of) a struct rt_context (see noocrun.c) */
    if (s1->dwarf) {
    ...
    *(addr_t *)section_ptr_add(s, PTR_SIZE) = s1->dwarf;
    ...
    if (s1->dwarf == 0 && s1->output_type == NOOC_OUTPUT_EXE)
    ...
    cstr_printf(&cstr, "__bt_init(__rt_info,%d);}",
        s1->output_type == NOOC_OUTPUT_DLL ? 0 : s1->rt_num_callers + 1);
    /* In case dlcose is called by application */
    cstr_printf(&cstr,
        "__attribute__((destructor)) static void __bt_exit_rt(){"
        "__bt_exit(__rt_info);}");
    nooc_compile_string_no_debug(s1, cstr.data);
    cstr_free(&cstr);
    set_local_sym(s1, &"___rt_info"[!s1->leading_underscore], s, o);
}
#endif /* def CONFIG_NOOC_BACKTRACE */

static void nooc_tcov_add_file(NOOCState *s1, const char *filename)
{
    CString cstr;
    void *ptr;
    char wd[1024];
...
#ifdef CONFIG_NOOC_BCHECK
        if (s1->do_bounds_check && s1->output_type != NOOC_OUTPUT_DLL) {
            nooc_add_support(s1, "bcheck.o");
# if !(TARGETOS_OpenBSD || TARGETOS_NetBSD)
            nooc_add_library_err(s1, "dl");
# endif
            lpthread = 1;
        }
#endif
#ifdef CONFIG_NOOC_BACKTRACE
        if (s1->do_backtrace) {
            if (s1->output_type & NOOC_OUTPUT_EXE)
                nooc_add_support(s1, "bt-exe.o");
            if (s1->output_type != NOOC_OUTPUT_DLL)
                nooc_add_support(s1, "bt-log.o");
            if (s1->output_type != NOOC_OUTPUT_MEMORY)
                nooc_add_btstub(s1);
        }
#endif
        if (lpthread)
            nooc_add_library_err(s1, "pthread");
        nooc_add_library_err(s1, "c");
#ifdef NOOC_LIBGCC
        if (!s1->static_link) {
            if (NOOC_LIBGCC[0] == '/')
                nooc_add_file(s1, NOOC_LIBGCC);
            else
                nooc_add_dll(s1, NOOC_LIBGCC, 0);
        }
#endif
#if defined NOOC_TARGET_ARM && TARGETOS_FreeBSD
        nooc_add_library_err(s1, "gcc_s"); // unwind code
#endif
        if (NOOC_LIBNOOC1[0])
            nooc_add_support(s1, NOOC_LIBNOOC1);
	...
	if (s1->output_type != NOOC_OUTPUT_MEMORY) {
	    ...
	    if (s1->output_type & NOOC_OUTPUT_DYN)
	        nooc_add_crt(s1, "crtendS.o");
	    else
	        nooc_add_crt(s1, "crtend.o");
            nooc_add_crt(s1, "crtn.o");
	    ...
	    if (s1->output_type == NOOC_OUTPUT_DLL)
                nooc_add_crt(s1, "crtend_so.o");
            else
                nooc_add_crt(s1, "crtend_android.o");
#else
            nooc_add_crt(s1, "crtn.o");
#endif
        }
...
#endif /* ndef NOOC_TARGET_PE */

/* add various standard linker symbols (must be done after the
   sections are filled (for example after allocating common
   symbols)) */
static void nooc_add_linker_symbols(NOOCState *s1)
{
    char buf[1024];
    int i;
    Section *s;
...
ST_FUNC void resolve_common_syms(NOOCState *s1)
{
    ...
    nooc_add_linker_symbols(s1);
}

#ifndef ELF_OBJ_ONLY
ST_FUNC void fill_got_entry(NOOCState *s1, ElfW_Rel *rel)
{
    int sym_index = ELFW(R_SYM) (rel->r_info);
    ElfW(Sym) *sym = &((ElfW(Sym) *) symtab_section->data)[sym_index];
...
ST_FUNC void fill_got(NOOCState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    int i;
...
static void fill_local_got_entries(NOOCState *s1)
{
    ElfW_Rel *rel;
    if (!s1->got->reloc)
        return;
	...
	if (ELFW(R_TYPE)(rel->r_info) == R_RELATIVE) {
	    ...
	    if (offset != rel->r_offset - s1->got->sh_addr)
	        nooc_error_noabort("fill_local_got_entries: huh?");
...
/* Bind symbols of executable: resolve undefined symbols from exported symbols
   in shared libraries */
static void bind_exe_dynsyms(NOOCState *s1, int is_PIE)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void bind_libs_dynsyms(NOOCState *s1)
{
    const char *name;
    int dynsym_index;
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(symtab_section, 1, sym, ElfW(Sym)) {
        ...
        if (sym->st_shndx != SHN_UNDEF) {
            ...
            if (esym->st_shndx == SHN_UNDEF) {
                ...
                if (ELFW(ST_BIND)(esym->st_info) != STB_WEAK)
                    nooc_warning("undefined dynamic symbol '%s'", name);
            }
        }
    }
}
...
static void export_global_syms(NOOCState *s1)
{
    int dynindex, index;
    const char *name;
    ElfW(Sym) *sym;
...
static int set_sec_sizes(NOOCState *s1)
{
    int i;
    Section *s;
    int textrel = 0;
...
/* Decide the layout of sections loaded in memory. This must be done before
   ...
   nobits */
static int sort_sections(NOOCState *s1, int *sec_order, Section *interp)
{
    Section *s;
    int i, j, k, f, f0, n;
...
static int layout_sections(NOOCState *s1, int *sec_order, struct dyn_inf *d)
{
    Section *s;
    ...
    if (d->roinf)
        ++phnum;
    d->phnum = phnum;
    d->phdr = nooc_mallocz(phnum * sizeof(ElfW(Phdr)));

    file_offset = 0;
    if (s1->output_format == NOOC_OUTPUT_FORMAT_ELF)
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(NOOCState *s1, struct dyn_inf *dyninf)
{
...
/* Remove gaps between RELX sections.
   ...
   The gaps are then filled with 0 in nooc_output_elf. The 0 is intepreted as
...
static void update_reloc_sections(NOOCState *s1, struct dyn_inf *dyninf)
{
    int i;
...
static int tidy_section_headers(NOOCState *s1, int *sec_order);
#endif /* ndef ELF_OBJ_ONLY */

/* Create an ELF file on disk.
   This function handle ELF specific layout requirements */
static int nooc_output_elf(NOOCState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
...
#elif defined NOOC_TARGET_ARM && defined NOOC_ARM_EABI
    ...
    if (file_type == NOOC_OUTPUT_OBJ) {
        ...
        if (s1->elf_entryname)
            ...
            ehdr.e_entry = get_sym_addr(s1, "_start", !!(file_type & NOOC_OUTPUT_EXE), 0);
...
static int nooc_output_binary(NOOCState *s1, FILE *f,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
static int nooc_write_elf_file(NOOCState *s1, const char *filename, int phnum,
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
    ...
    if (fd < 0 || (f = fdopen(fd, "wb")) == NULL)
        return nooc_error_noabort("could not write '%s: %s'", filename, strerror(errno));
    if (s1->verbose)
        printf("<- %s\n", filename);
#ifdef NOOC_TARGET_COFF
    if (s1->output_format == NOOC_OUTPUT_FORMAT_COFF)
        nooc_output_coff(s1, f);
    else
#endif
    if (s1->output_format == NOOC_OUTPUT_FORMAT_ELF)
        ret = nooc_output_elf(s1, f, phnum, phdr, file_offset, sec_order);
    else
        ret = nooc_output_binary(s1, f, sec_order);
    fclose(f);
...
static int tidy_section_headers(NOOCState *s1, int *sec_order)
{
    ...
    snew = nooc_malloc(s1->nb_sections * sizeof(snew[0]));
    backmap = nooc_malloc(s1->nb_sections * sizeof(backmap[0]));
...
#ifdef NOOC_TARGET_ARM
static void create_arm_attribute_section(NOOCState *s1)
{
...
#if TARGETOS_OpenBSD || TARGETOS_NetBSD
static Section *create_bsd_note_section(NOOCState *s1,
					const char *name,
					const char *value)
{
...
static void alloc_sec_names(NOOCState *s1, int is_obj);

/* Output an elf, coff or binary file */
/* XXX: suppress unneeded sections */
static int elf_output_file(NOOCState *s1, const char *filename)
{
	...
	resolve_common_syms(s1);
	    ...
	    s1->dynsym->sh_info = 1;
            ...
            got_sym = build_got(s1);
            if (file_type & NOOC_OUTPUT_EXE) {
                bind_exe_dynsyms(s1, file_type & NOOC_OUTPUT_DYN);
                if (s1->nb_errors)
                    goto the_end;
            }
    ...
    sec_order = nooc_malloc(sizeof(int) * 2 * s1->nb_sections);
    ...
    file_offset = layout_sections(s1, sec_order, &dyninf);
        ...
        if (dynamic) {
            ...
            if (file_type == NOOC_OUTPUT_EXE
                || (RELOCATE_DLLPLT && (file_type & NOOC_OUTPUT_DYN)))
                relocate_plt(s1);
        ...
        /* Perform relocation to GOT or PLT entries */
        if (file_type == NOOC_OUTPUT_EXE && s1->static_link)
            fill_got(s1);
        else if (s1->got)
    ...
    ret = nooc_write_elf_file(s1, filename, dyninf.phnum, dyninf.phdr, file_offset, sec_order);
 the_end:
    nooc_free(sec_order);
    nooc_free(dyninf.phdr);
    return ret;
}
#endif /* ndef ELF_OBJ_ONLY */
...
static int elf_output_obj(NOOCState *s1, const char *filename)
{
    Section *s;
    ...
    /* Create the ELF file with name 'filename' */
    ret = nooc_write_elf_file(s1, filename, 0, NULL, file_offset, NULL);
    return ret;
}
...
{
    if (s->test_coverage)
        nooc_tcov_add_file(s, filename);
    if (s->output_type == NOOC_OUTPUT_OBJ)
        return elf_output_obj(s, filename);
#ifdef NOOC_TARGET_PE
    return  pe_output_file(s, filename);
#elif NOOC_TARGET_MACHO
...
ST_FUNC int nooc_load_object_file(NOOCState *s1,
                                int fd, unsigned long file_offset)
{
    ...
    lseek(fd, file_offset, SEEK_SET);
    if (nooc_object_type(fd, &ehdr) != AFF_BINTYPE_REL)
        goto invalid;
    /* test CPU specific stuff */
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_NOOC_TARGET) {
invalid:
        return nooc_error_noabort("invalid object file");
    }
    /* read sections */
    shdr = load_data(fd, file_offset + ehdr.e_shoff,
                     sizeof(ElfW(Shdr)) * ehdr.e_shnum);
    sm_table = nooc_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_SYMTAB) {
            if (symtab) {
                nooc_error_noabort("object must contain only one symtab");
                goto the_end;
            }
    ...
    found:
        if (sh->sh_type != s->sh_type
            ...
            nooc_error_noabort("invalid section type");
            goto the_end;
        }
    ...
    /* resolve symbols */
    old_to_new_syms = nooc_mallocz(nb_syms * sizeof(int));
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
	    ...
	    for (rel = (ElfW_Rel *) s->data + (offset / sizeof(*rel));
		 ...
		 rel++) {
                ...
                invalid_reloc:
                    nooc_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x",
...
static int nooc_load_alacarte(NOOCState *s1, int fd, int size, int entrysize)
{
    ...
    do {
        ...
        for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p)+1) {
            ...
            if (len <= 0 || memcmp(hdr.ar_fmag, ARFMAG, 2)) {
                nooc_error_noabort("invalid archive");
                goto the_end;
            }
            off += len;
            if (s1->verbose == 2)
                printf("   -> %s\n", hdr.ar_name);
            if (nooc_load_object_file(s1, fd, off) < 0)
                goto the_end;
            ++bound;
        }
    } while(bound);
...
ST_FUNC int nooc_load_archive(NOOCState *s1, int fd, int alacarte)
{
    ...
    for(;;) {
        ...
        if (len == 0)
            return 0;
        if (len < 0)
            return nooc_error_noabort("invalid archive");
        file_offset += len;
        ...
        if (alacarte) {
            ...
            if (!strcmp(hdr.ar_name, "/"))
                return nooc_load_alacarte(s1, fd, size, 4);
            if (!strcmp(hdr.ar_name, "/SYM64/"))
                return nooc_load_alacarte(s1, fd, size, 8);
        } else if (nooc_object_type(fd, &ehdr) == AFF_BINTYPE_REL) {
            if (s1->verbose == 2)
                printf("   -> %s\n", hdr.ar_name);
            if (nooc_load_object_file(s1, fd, file_offset) < 0)
                return -1;
        }
        file_offset += size;
    }
...
static void set_ver_to_ver(NOOCState *s1, int *n, int **lv, int i, char *lib, char *version)
{
    while (i >= *n) {
        *lv = nooc_realloc(*lv, (*n + 1) * sizeof(**lv));
        (*lv)[(*n)++] = -1;
    }
    if ((*lv)[i] == -1) {
        ...
        if (v == nb_sym_versions) {
            sym_versions = nooc_realloc (sym_versions,
                                        (v + 1) * sizeof(*sym_versions));
            sym_versions[v].lib = nooc_strdup(lib);
            sym_versions[v].version = nooc_strdup(version);
...
/* Associates symbol SYM_INDEX (in dynsymtab) with sym-version index
   VERNDX.  */
static void
set_sym_version(NOOCState *s1, int sym_index, int verndx)
{
    if (sym_index >= nb_sym_to_version) {
        int newelems = sym_index ? sym_index * 2 : 1;
        sym_to_version = nooc_realloc(sym_to_version,
...
static void store_version(NOOCState *s1, struct versym_info *v, char *dynstr)
{
    char *lib, *version;
    uint32_t next;
    int i;
...
ST_FUNC int nooc_load_dll(NOOCState *s1, int fd, const char *filename, int level)
{
    ...
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_NOOC_TARGET) {
        return nooc_error_noabort("bad architecture");
    }
    ...
    soname = nooc_basename(filename);
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++)
        if (dt->d_tag == DT_SONAME)
            soname = dynstr + dt->d_un.d_val;

    /* if the dll is already loaded, do not load it */
    if (nooc_add_dllref(s1, soname, level)->found)
        goto ret_success;

    if (v.nb_versyms != nb_syms)
        nooc_free (v.versym), v.versym = NULL;
    else
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++)
        if (dt->d_tag == DT_RPATH)
            nooc_add_library_path(s1, dynstr + dt->d_un.d_val);
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        ...
        case DT_NEEDED:
            ...
            if (nooc_add_dllref(s1, name, -1))
                continue;
            if (nooc_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
                ret = nooc_error_noabort("referenced dll '%s' not found", name);
                goto the_end;
            }
        }
    }
...
static int ld_inp(NOOCState *s1)
{
    char b;
    if (s1->cc != -1) {
        int c = s1->cc;
...
static int ld_next(NOOCState *s1, char *name, int name_size)
{
    int c, d, ch;
    char *q;
...
static int ld_add_file(NOOCState *s1, const char filename[])
{
    if (filename[0] == '/') {
        if (CONFIG_SYSROOT[0] == '\0'
            && nooc_add_file_internal(s1, filename, AFF_TYPE_BIN) == 0)
            return 0;
        filename = nooc_basename(filename);
    }
    return nooc_add_dll(s1, filename, 0);
}

static int ld_add_file_list(NOOCState *s1, const char *cmd, int as_needed)
{
    ...
    if (t != '(') {
        ret = nooc_error_noabort("( expected");
    ...
    for(;;) {
        libname[0] = '\0';
        if (t == LD_TOK_EOF) {
            ret = nooc_error_noabort("unexpected end of file");
            goto lib_parse_error;
        } else if (t == ')') {
            break;
        } else if (t == '-') {
            ...
            if ((t != LD_TOK_NAME) || (filename[0] != 'l')) {
                ret = nooc_error_noabort("library name expected");
                goto lib_parse_error;
            }
            ...
            ret = nooc_error_noabort("filename expected");
            goto lib_parse_error;
        }
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) {
                ...
                if (group) {
                    ...
                    dynarray_add(&libs, &nblibs, nooc_strdup(filename));
                    if (libname[0] != '\0')
                        dynarray_add(&libs, &nblibs, nooc_strdup(libname));
                }
            }
        }
        t = ld_next(s1, filename, sizeof(filename));
...
ST_FUNC int nooc_load_ldscript(NOOCState *s1, int fd)
{
    ...
    for(;;) {
        ...
        if (!strcmp(cmd, "INPUT") ||
            ...
            t = ld_next(s1, cmd, sizeof(cmd));
            if (t != '(')
                return nooc_error_noabort("( expected");
            for(;;) {
                t = ld_next(s1, filename, sizeof(filename));
                if (t == LD_TOK_EOF) {
                    return nooc_error_noabort("unexpected end of file");
                } else if (t == ')') {
                    break;
                }
            }
