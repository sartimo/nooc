...
static void gen_inline_functions(NOOCState *s);
static void free_inline_functions(NOOCState *s);
...
static int gind()
{
  ...
  CODE_ON();
  if (debug_modes)
    nooc_tcov_block_begin(nooc_state);
  return t;
}
...
/* compiling intel long double natively */
    ...
    && (defined NOOC_TARGET_I386 || defined NOOC_TARGET_X86_64)
# define NOOC_IS_NATIVE_387
#endif
...
{
    if (vtop != vstack - 1)
        nooc_error("internal compiler error: vstack leak (%d)",
                  (int)(vtop - vstack + 1));
}
...
/* ------------------------------------------------------------------------- */
/* initialize vstack and types.  This must be done also for nooc -E */
...
{
    nooc_debug_end(s1); /* just in case of errors: free memory */
                            ...
                            int can_add_underscore)
{
    ...
    if (!sym->c) {
        ...
        if (nooc_state->leading_underscore && can_add_underscore) {
            buf1[0] = '_';
        ...
        if (debug_modes)
            nooc_debug_extern_sym(nooc_state, sym, sh_num, sym_bind, sym_type);
...
static Sym *__sym_malloc(void)
{
    ...
    sym_pool = nooc_malloc(SYM_POOL_NB * sizeof(Sym));
...
ST_FUNC void label_pop(Sym **ptop, Sym *slast, int keep)
{
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            nooc_warning_c(warn_all)("label '%s' declared but not used", get_tok_str(s->v, NULL));
        } else if (s->r == LABEL_FORWARD) {
                nooc_error("label '%s' used but not defined",
...
static void patch_type(Sym *sym, CType *type)
{
    if (!(type->t & VT_EXTERN) || IS_ENUM_VAL(sym->type.t)) {
        if (!(sym->type.t & VT_EXTERN))
            nooc_error("redefinition of '%s'", get_tok_str(sym->v, NULL));
    ...
    if (!is_compatible_types(&sym->type, type)) {
        nooc_error("incompatible types for redefinition of '%s'",
                  get_tok_str(sym->v, NULL));
        ...
        if ((type->t & VT_STATIC) && !static_proto
            ...
            nooc_warning("static storage ignored for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
        ...
        if ((type->t ^ sym->type.t) & VT_STATIC)
            nooc_warning("storage mismatch for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
    }
}
...
static void patch_storage(Sym *sym, AttributeDef *ad, CType *type)
{
    ...
    if (sym->a.dllimport != ad->a.dllimport)
        nooc_error("incompatible dll linkage for redefinition of '%s'",
            get_tok_str(sym->v, NULL));
#endif
...
{
#ifdef CONFIG_NOOC_BCHECK
    if (nooc_state->do_bounds_check && !keep && (local_scope || !func_var))
        add_local_bounds(local_stack, b);
#endif
    if (debug_modes)
        nooc_add_debug_info (nooc_state, !local_scope, local_stack, b);
        ...
        case TOK_UMOD:
            ...
            if (l2 == 0) {
                if (CONST_WANTED && !NOEVAL_WANTED)
                    nooc_error("division by zero in constant");
                goto general_case;
            }
            switch(op) {
...
#if defined NOOC_TARGET_X86_64 || defined NOOC_TARGET_I386
...
void gen_negf(int op)
{
    /* In IEEE negate(x) isn't subtract(0,x).  Without NaNs it's
       ...
       to do some type reinterpretation for this, which NOOC can do
       only via memory.  */
    ...
    if (bt1 == VT_VOID || bt2 == VT_VOID) {
        ...
        /* If one is a null ptr constant the result type is the other.  */
            ...
            if ((op == '?' || TOK_ISCOND(op))
              ...
              nooc_warning("pointer/integer mismatch in %s",
...
redo:
	...
	goto redo;
        ...
        nooc_error("invalid operand types for binary operation");
    } else if (bt1 == VT_PTR || bt2 == VT_PTR) {
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            if (op != '-')
                nooc_error("cannot use pointers here");
            ...
            gen_op('*');
#ifdef CONFIG_NOOC_BCHECK
            if (nooc_state->do_bounds_check && !CONST_WANTED) {
        ...
        if (is_float(combtype.t)
            && op != '+' && op != '-' && op != '*' && op != '/'
            && !TOK_ISCOND(op))
            nooc_error("invalid operands for binary operation");
...
#if defined NOOC_TARGET_ARM64 || defined NOOC_TARGET_RISCV64 || defined NOOC_TARGET_ARM
...
#if defined NOOC_TARGET_ARM64 || defined NOOC_TARGET_RISCV64
...
error:
        ...
        c = (vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST;
#if !defined NOOC_IS_NATIVE && !defined NOOC_IS_NATIVE_387
        ...
        if (c) {
            ...
            && (vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM))
                == (VT_CONST | VT_SYM)) {
            /* addresses are considered non-zero (see nooctest.c:sinit23) */
            vtop->r = VT_CONST;
            vtop->c.i = 1;
            goto done;
        }
        ...
        if (IS_ENUM(type->t) && type->ref->c < 0)
            nooc_error("cast to incomplete type");

        /* same size and no sign conversion needed */
        if (ds == ss && ds >= 4)
            goto done;
        if (dbt_bt == VT_PTR || sbt_bt == VT_PTR) {
            nooc_warning("cast between pointer and integer of different size");
            if (sbt_bt == VT_PTR) {
        ...
        if (ds == 8) {
            ...
            if (sbt & VT_UNSIGNED) {
#if defined(NOOC_TARGET_RISCV64)
        ...
        if (ds >= ss)
            goto done;
#if defined NOOC_TARGET_I386 || defined NOOC_TARGET_X86_64 || defined NOOC_TARGET_ARM64
        if (ss == 4) {
            gen_cvt_csti(dbt);
            goto done;
        }
...
static void verify_assign_cast(CType *dt)
{
    ...
    sbt = st->t & VT_BTYPE;
    if (dt->t & VT_CONSTANT)
        nooc_warning("assignment of read-only location");
    switch(dbt) {
    case VT_VOID:
        if (sbt != dbt)
            nooc_error("assignment to void expression");
        break;
    case VT_PTR:
        ...
        if (is_integer_btype(sbt)) {
            nooc_warning("assignment makes pointer from integer without a cast");
            break;
        }
		...
		/* Like GCC don't warn by default for merely changes
		   ...
		   and signed int targets.  */
                ...
                nooc_warning("assignment from incompatible pointer type");
                break;
            }
        }
        if (qualwarn)
            nooc_warning_c(warn_discarded_qualifiers)("assignment discards qualifiers from pointer target type");
        break;
    case VT_BYTE:
    case VT_SHORT:
    case VT_INT:
    case VT_LLONG:
        if (sbt == VT_PTR || sbt == VT_FUNC) {
            nooc_warning("assignment makes integer from pointer without a cast");
    ...
    if (sbt == VT_STRUCT) {
...
#ifdef NOOC_TARGET_NATIVE_STRUCT_COPY
        if (1
#ifdef CONFIG_NOOC_BCHECK
            && !nooc_state->do_bounds_check
#endif
            ) {
        ...
        --vtop;
            ...
            if (delayed_cast) {
                ...
                //nooc_warning("deley cast %x -> %x", sbt, dbt);
...
redo:
	...
	case TOK_CLEANUP2:
	{
	    ...
	    s = sym_find(tok);
	    if (!s) {
	        nooc_warning_c(warn_implicit_function_declaration)(
                    "implicit declaration of function '%s'", get_tok_str(tok, &tokc));
	        s = external_global_sym(tok, &func_old_type);
            } else if ((s->type.t & VT_BTYPE) != VT_FUNC)
                nooc_error("'%s' is not declared as function", get_tok_str(tok, &tokc));
	    ad->cleanup_func = s;
	    next();
            skip(')');
	    break;
	    ...
	    astr = parse_mult_str("section name")->data;
            ad->section = find_section(nooc_state, astr);
            skip(')');
            break;
        case TOK_ALIAS1:
        case TOK_ALIAS2:
	...
	case TOK_VISIBILITY2:
        ...
        case TOK_ALIGNED2:
            if (tok == '(') {
                ...
                n = expr_const();
                if (n <= 0 || (n & (n - 1)) != 0) 
                    nooc_error("alignment must be a positive power of two");
                skip(')');
            } else {
                n = MAX_ALIGN;
            }
	    ...
	    if (n != 1 << (ad->a.aligned - 1))
	      nooc_error("alignment of %d is larger than implemented", n);
            break;
        case TOK_PACKED1:
        case TOK_PACKED2:
            ad->a.packed = 1;
        ...
        case TOK_UNUSED1:
        case TOK_UNUSED2:
            /* currently, no need to handle it because nooc does not
        ...
        case TOK_MODE:
            ...
            switch(tok) {
                ...
                case TOK_MODE_word:
                    ...
                    break;
                default:
                    nooc_warning("__mode__(%s) not supported\n", get_tok_str(tok, NULL));
                    break;
            }
            next();
            skip(')');
        ...
        case TOK_DLLIMPORT:
            ...
            break;
        default:
            nooc_warning_c(warn_unsupported)("'%s' attribute ignored", get_tok_str(t, NULL));
            /* skip parameters */
            if (tok == '(') {
                int parenthesis = 0;
                do {
...
static Sym * find_field (CType *type, int v, int *cumofs)
{
    ...
    if (!(v & SYM_FIELD)) { /* top-level call */
        ...
        if (v < TOK_UIDENT)
            expect("field name");
        if (s->c < 0)
            nooc_error("dereferencing incomplete type '%s'",
    ...
    if (!(v & SYM_FIELD))
        nooc_error("field not found: %s", get_tok_str(v, NULL));
    return s;
}
    ...
    while ((s = s->next) != NULL) {
        ...
        if (v < SYM_FIRST_ANOM) {
            ...
            if (check && (ts->tok & SYM_FIELD))
                nooc_error("duplicate member '%s'", get_tok_str(v, NULL));
            ts->tok ^= SYM_FIELD;
...
static void struct_layout(CType *type, AttributeDef *ad)
{
    ...
    int pcc = !nooc_state->ms_bitfields;
    int pragma_pack = *nooc_state->pack_stack_ptr;
    Sym *f;
    ...
    if (tok != '{') {
        ...
        if (s && (s->sym_scope == local_scope || tok != '{')) {
            ...
            if (u == VT_ENUM && IS_ENUM(s->type.t))
                goto do_decl;
            nooc_error("redefinition of '%s'", get_tok_str(v, NULL));
        }
    } else {
        v = anon_sym++;
    }
...
do_decl:
    ...
    if (tok == '{') {
        next();
        if (s->c != -1)
            nooc_error("struct/union/enum already defined");
        s->c = -2;
        /* cannot be empty */
	    ...
	    CType t;
            ...
            for(;;) {
                ...
                ss = sym_find(v);
                if (ss && !local_stack)
                    nooc_error("redefinition of enumerator '%s'",
                              get_tok_str(v, NULL));
                next();
                if (tok == '=') {
                    next();
		    ...
		    continue;
                ...
                while (1) {
		    if (flexible)
		        nooc_error("flexible array member '%s' not at the end of struct",
                              get_tok_str(v, NULL));
                    bit_size = -1;
                    v = 0;
                    type1 = btype;
			...
			if (tok != ';')
			    ...
			    if ((u == VT_STRUCT) && (type1.t & VT_ARRAY) && c)
			        ...
			        nooc_error("field '%s' has incomplete type",
                                      get_tok_str(v, NULL));
                        }
                        if ((type1.t & VT_BTYPE) == VT_FUNC ||
			    (type1.t & VT_BTYPE) == VT_VOID ||
                            (type1.t & VT_STORAGE))
                            nooc_error("invalid type for '%s'", 
                                  get_tok_str(v, NULL));
                    }
                    if (tok == ':') {
                        next();
                        ...
                        if (bit_size < 0)
                            nooc_error("negative width in bit-field '%s'", 
                                  get_tok_str(v, NULL));
                        if (v && bit_size == 0)
                            nooc_error("zero width for bit-field '%s'", 
                                  get_tok_str(v, NULL));
			parse_attribute(&ad1);
                    }
                    ...
                    if (bit_size >= 0) {
                        ...
                        if (bt != VT_INT && 
                            ...
                            bt != VT_BOOL &&
                            bt != VT_LLONG)
                            nooc_error("bitfields must have scalar type");
                        bsize = size * 8;
                        if (bit_size > bsize) {
                            nooc_error("width of '%s' exceeds its type",
                                  get_tok_str(v, NULL));
                            ...
                            nooc_error("field width 64 not implemented");
                        } else {
	    ...
	    parse_attribute(&ad);
            if (ad.cleanup_func) {
                nooc_warning("attribute '__cleanup__' ignored on type");
            }
	    check_fields(type, 1);
	    check_fields(type, 0);
            struct_layout(type, &ad);
	    if (debug_modes)
		nooc_debug_fix_anon(nooc_state, type);
        }
    }
}
...
static int parse_btype(CType *type, AttributeDef *ad, int ignore_label)
{
    ...
    while(1) {
        ...
        basic_type1:
            if (u == VT_SHORT || u == VT_LONG) {
                if (st != -1 || (bt != -1 && bt != VT_INT))
                    tmbt: nooc_error("too many basic types");
                st = u;
            } else {
        ...
        case TOK_BOOL:
            ...
            goto basic_type;
        case TOK_COMPLEX:
            nooc_error("_Complex is not yet supported");
        case TOK_FLOAT:
            u = VT_FLOAT;
            goto basic_type;
        case TOK_DOUBLE:
        ...
        case TOK_SIGNED3:
            if ((t & (VT_DEFSIGN|VT_UNSIGNED)) == (VT_DEFSIGN|VT_UNSIGNED))
                nooc_error("signed and unsigned modifier");
            t |= VT_DEFSIGN;
            next();
            typespec_found = 1;
            break;
       ...
       storage:
            if (t & (VT_EXTERN|VT_STATIC|VT_TYPEDEF) & ~g)
                nooc_error("multiple storage classes");
            t |= g;
            next();
            break;
        case TOK_INLINE1:
        ...
        case TOK_THREAD_LOCAL:
            nooc_error("_Thread_local is not implemented");
        default:
            if (typespec_found)
                goto the_end;
            s = sym_find(tok);
...
the_end:
    if (nooc_state->char_is_unsigned) {
...
#ifdef NOOC_USING_DOUBLE_FOR_LDOUBLE
    if (bt == VT_LDOUBLE)
            ...
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    ...
                    if ((pt.t & VT_BTYPE) == VT_VOID)
                        nooc_error("parameter declared as void");
                    if (n == 0)
                        n = SYM_FIELD;
                } else {
                    n = tok;
...
check:
            if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
                n = vtop->c.i;
                if (n < 0)
                    nooc_error("invalid array size");
            } else {
                if (!is_integer_btype(vtop->type.t & VT_BTYPE))
                    nooc_error("size of variable length array should be an integer");
                n = 0;
                t1 = VT_VLA;
            }
        }
        ...
        if ((type->t & VT_BTYPE) == VT_FUNC)
            nooc_error("declaration of an array of functions");
        if ((type->t & VT_BTYPE) == VT_VOID
            || type_size(type, &align) < 0)
            nooc_error("declaration of an array of incomplete type elements");
        ...
        if (t1 & VT_VLA) {
            if (n < 0) {
		if  (td & TYPE_NEST)
                    nooc_error("need explicit inner array size in VLAs");
	    }
	    else {
...
static void gfunc_param_typed(Sym *func, Sym *arg)
{
    ...
    if (func_type == FUNC_OLD ||
        ...
        nooc_error("too many arguments to function");
    } else {
        type = arg->type;
...
static void parse_atomic(int atok)
{
    ...
    static const char *const templates[] = {
        ...
        /* keep in order of appearance in nooctok.h: */
        /* __atomic_store */            "alm.?",
        /* __atomic_load */             "Asm.v",
    ...
    for (arg = 0;;) {
        ...
        case 'A':
            ...
            /* if (!(atom->t & VT_ATOMIC))
                nooc_warning("pointer target declaration is missing '_Atomic'"); */
            break;

        case 'p':
            if ((vtop->type.t & VT_BTYPE) != VT_PTR
             || type_size(pointed_type(&vtop->type), &align) != size)
                nooc_error("pointer target type mismatch in argument %d", arg + 1);
...
ST_FUNC void unary(void)
{
    ...
    if (debug_modes)
        nooc_debug_line(nooc_state), nooc_tcov_check_line (nooc_state, 1);
 ...
 push_tokc:
    ...
    case_TOK_STR:
        ...
        t = char_type.t;
    str_init:
        if (nooc_state->warn_write_strings & WARN_ON)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
    ...
    case '(':
	    ...
	    int saved_nocode_wanted = nocode_wanted;
            if (CONST_WANTED && !NOEVAL_WANTED)
                expect("constant");
            if (0 == local_scope)
                nooc_error("statement expression outside of function");
            /* save all registers */
            save_regs(0);
    ...
    case '+':
        ...
        unary();
        if ((vtop->type.t & VT_BTYPE) == VT_PTR)
            nooc_error("pointer not accepted for unary plus");
    ...
    case TOK_builtin_return_address:
        {
            ...
            level = expr_const64();
            if (level < 0) {
                nooc_error("%s only takes positive integers",
    ...
    case TOK_builtin_va_start:
        ...
        if (r == VT_LLOCAL)
            r = VT_LOCAL;
        if (r != VT_LOCAL)
            nooc_error("__builtin_va_start expects a local variable");
        gen_va_start();
	vstore();
        break;
#endif
	...
	if (!str) {
	    ...
	    nooc_error("type '%s' does not match any association", buf);
	}
	begin_macro(str, 1);
	next();
	expr_eq();
...
special_math_val:
    ...
    while (1) {
	    ...
	    s = find_field(&vtop->type, tok, &cumofs);
            ...
            if (sa)
                nooc_error("too few arguments to function");
            skip(')');
            gfunc_call(nb_args);
            ...
            if (ret_nregs < 0) {
                ...
                /* Promote char/short return values. This is matters only
                   for calling function that were not compiled by NOOC and
            ...
            if (s->f.func_noreturn) {
                if (debug_modes)
	            nooc_tcov_block_end(nooc_state, -1);
                CODE_OFF();
	    }
        } else {
            break;
...
static void expr_cond(void)
{
	...
	c = condition_3way();
        ...
        if (c < 0 && is_cond_bool(vtop) && is_cond_bool(&sv)) {
            ...
            gvtst_set(1, t2);
            gen_cast(&type);
            //  nooc_warning("two conditions expr_cond");
            return;
        }
...
ST_FUNC int expr_const(void)
{
    ...
    if (c != wc && (unsigned)c != wc)
        nooc_error("constant exceeds 32 bit");
    return c;
}
...
static void check_func_return(void)
{
    ...
    if (!strcmp (funcname, "main")
        ...
        nooc_warning("function might return no value: '%s'", funcname);
    }
}
...
again:
    ...
    if (debug_modes)
        nooc_tcov_check_line (nooc_state, 0), nooc_tcov_block_begin (nooc_state);
    ...
    if (t == TOK_IF) {
        ...
        if (debug_modes)
            nooc_debug_stabn(nooc_state, N_LBRAC, ind - func_ind);
        new_scope(&o);
        ...
        if (debug_modes)
            nooc_debug_stabn(nooc_state, N_RBRAC, ind - func_ind);
        if (local_scope)
            next();
        ...
        if (tok != ';') {
            ...
            if (b) {
                ...
                if (vtop->type.t != VT_VOID)
                    nooc_warning("void function returns a value");
                vtop--;
            }
        } else if (b) {
            nooc_warning("'return' with no value");
            b = 0;
        }
	...
	gsym_addr(c, d);
        ...
        for (b = 1; b < sw->n; b++)
            if (sw->sv.type.t & VT_UNSIGNED
                ...
                nooc_error("duplicate case value");
        vpushv(&sw->sv);
        gv(RC_INT);
    ...
    skip_switch:
        ...
        struct case_t *cr = nooc_malloc(sizeof(struct case_t));
        if (!cur_switch)
            expect("switch");
	    ...
	    next();
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    nooc_error("duplicate label '%s'", get_tok_str(s->v, NULL));
                s->r = LABEL_DEFINED;
		if (s->next) {
    ...
    block_after_label:
            ...
            if (debug_modes)
                nooc_tcov_reset_ind(nooc_state);
            vla_restore(cur_scope->vla.loc);
            if (tok != '}')
                goto again;
            /* we accept this, but it is a mistake */
            nooc_warning_c(warn_all)("deprecated use of label at end of compound statement");
    ...
    if (debug_modes)
        nooc_tcov_check_line (nooc_state, 0), nooc_tcov_block_end (nooc_state, 0);
}
...
static void skip_or_save_block(TokenString **str)
{
	...
	if (t == TOK_EOF) {
	     if (str || level > 0)
	       nooc_error("unexpected end of file");
	     else
	       break;
	}
	if (str)
    ...
    case EXPR_CONST:
	   ...
	   (compound literals).  */
        if (((vtop->r & (VT_VALMASK | VT_LVAL)) != VT_CONST
            ...
            nooc_error("initializer element is not constant");
        break;
    case EXPR_ANY:
        expr_eq();
        break;
...
static void init_assert(init_params *p, int offset)
{
    if (p->sec ? !NODATA_WANTED && offset > p->sec->data_offset
        ...
        nooc_internal_error("initializer overflow");
}
#else
...
static void decl_design_flex(init_params *p, Sym *ref, int index)
{
    if (ref == p->flex_array_ref) {
        ...
        nooc_error("flexible array has zero size in this context");
}
                           ...
                           Sym **cur_field, int flags, int al)
{
    ...
    while (nb_elems == 1 && (tok == '[' || tok == '.')) {
        if (tok == '[') {
	        ...
	        nooc_error("index exceeds array bounds or range is empty");
            if (cur_field)
    ...
    no_designator:
	    ...
	    index = (*cur_field)->c;
            ...
            if (index >= s->c)
                nooc_error("too many initializers");
            type = pointed_type(type);
            elem_size = type_size(type, &align);
            c += index * elem_size;
        } else {
...
static void init_putv(init_params *p, CType *type, unsigned long c)
{
    ...
    if (sec) {
        ...
        if ((vtop->r & VT_SYM)
            ...
            nooc_error("initializer element is not computable at load time");
	...
	if ((vtop->r & (VT_SYM|VT_CONST)) == (VT_SYM|VT_CONST) &&
	    ...
	    ssec = nooc_state->sections[esym->st_shndx];
		...
		*(char *)ptr = val;
		...
		break;
	    case VT_LDOUBLE:
#if defined NOOC_IS_NATIVE_387
                /* Host and target platform may be different but both have x87.
                   On windows, nooc does not use VT_LDOUBLE, except when it is a
                   cross compiler.  In this case a mingw gcc as host compiler
                   comes here with 10-byte long doubles, while msvc or nooc won't.
                   nooc itself can still translate by asm.
...
#ifndef NOOC_CROSS_TEST
                else
                    nooc_error("can't cross compile long double constants");
#endif
		break;

#if PTR_SIZE == 8
            /* intptr_t may need a reloc too, see nooctest.c:relocation_test() */
	    case VT_LLONG:
	    case VT_PTR:
	        if (vtop->r & VT_SYM)
...
#endif
	    default:
                //nooc_internal_error("unexpected type");
                break;
	    }
	}
        vtop--;
    ...
    if (type->t & VT_ARRAY) {
	    ...
	    len = 0;
            ...
            if (size1 != (tok == TOK_STR ? 1 : sizeof(nwchar_t)))
              nooc_error("unhandled string literal merging");
            ...
            if (!(flags & DIF_SIZE_ONLY)) {
                ...
                if (len < nb)
                    nb = len;
                if (len > nb)
                  nooc_warning("initializer-string for array is too long");
                                   ...
                                   int has_init, int v, int global)
{
    ...
    int saved_nocode_wanted = nocode_wanted;
#ifdef CONFIG_NOOC_BCHECK
    int bcheck = nooc_state->do_bounds_check && !NODATA_WANTED;
#endif
    init_params p = {0};
	...
	sym = NULL;
		...
		sec = data_section;
                /*if (nooc_state->g_debug & 4)
                    nooc_warning("rw data: %s", get_tok_str(v, 0));*/
            } else if (nooc_state->nocommon)
                sec = bss_section;
        }
...
#if defined NOOC_TARGET_PE && defined NOOC_TARGET_X86_64
...
static void gen_function(Sym *sym)
{
    ...
    if (sym->type.ref->f.func_ctor)
        add_array (nooc_state, ".init_array", sym->c);
    if (sym->type.ref->f.func_dtor)
        add_array (nooc_state, ".fini_array", sym->c);

    /* put debug symbol */
    nooc_debug_funcstart(nooc_state, sym);

    /* push a dummy symbol to enable local sym storage */
    sym_push2(&local_stack, SYM_FIELD, 0, 0);
    local_scope = 1; /* for function parameters */
    gfunc_prolog(sym);
    nooc_debug_prolog_epilog(nooc_state, 0);
    ...
    nooc_debug_prolog_epilog(nooc_state, 1);
    gfunc_epilog();

    /* end of function */
    nooc_debug_funcend(nooc_state, ind - func_ind);
    ...
    do {
        ...
        for (i = 0; i < s->nb_inline_fns; ++i) {
            ...
            if (sym && (sym->c || !(sym->type.t & VT_INLINE))) {
                ...
                fn->sym = NULL;
                nooc_debug_putfile(s, fn->filename);
...
static int decl(int l)
{
    ...
    while (1) {
		...
		v = btype.ref->v;
		if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) >= SYM_FIRST_ANOM)
        	    nooc_warning("unnamed struct/union that defines no instances");
                next();
                continue;
	    }
            if (IS_ENUM(btype.t)) {
	    ...
	    ad = adbase;
            ...
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                if ((type.t & VT_STATIC) && (l != VT_CONST))
                    nooc_error("function without file scope cannot be static");
...
#if defined NOOC_TARGET_MACHO || defined TARGETOS_ANDROID
                ...
                nooc_warning("type defaults to int");
            }
            ...
            if (ad.a.dllimport || ad.a.dllexport) {
                if (type.t & VT_STATIC)
                    nooc_error("cannot have dll linkage with static");
                if (type.t & VT_TYPEDEF) {
                    nooc_warning("'%s' attribute ignored for typedef",
            ...
            if (tok == '{') {
                if (l != VT_CONST)
                    nooc_error("cannot use local functions");
                if ((type.t & VT_BTYPE) != VT_FUNC)
                ...
                if (sym->type.t & VT_INLINE) {
                    struct InlineFunc *fn;
                    fn = nooc_malloc(sizeof *fn + strlen(file->filename));
                    strcpy(fn->filename, file->filename);
                    fn->sym = sym;
		    skip_or_save_block(&fn->func_str);
                    dynarray_add(&nooc_state->inline_fns,
				 &nooc_state->nb_inline_fns, fn);
                } else {
		...
		if (l == VT_CMP) {
			...
			if ((sym->v & ~SYM_FIELD) == v)
			    goto found;
		    nooc_error("declaration for parameter '%s' but no such parameter",
			      get_tok_str(v, NULL));
                found:
		    if (type.t & VT_STORAGE) /* 'register' is okay */
		        nooc_error("storage class specified for '%s'",
				  get_tok_str(v, NULL));
		    if (sym->type.t != VT_VOID)
		        nooc_error("redefinition of parameter '%s'",
				  get_tok_str(v, NULL));
		    ...
		    sym->type = type;
                    ...
                    if (sym && sym->sym_scope == local_scope) {
                        if (!is_compatible_types(&sym->type, &type)
                            ...
                            nooc_error("incompatible redefinition of '%s'",
                                get_tok_str(v, NULL));
                        sym->type = type;
                    } else {
                    ...
                    if (debug_modes)
                        nooc_debug_typedef (nooc_state, sym);
		} else if ((type.t & VT_BTYPE) == VT_VOID
			   && !(type.t & VT_EXTERN)) {
		    nooc_error("declaration of void object");
                } else {
                    r = 0;
                    if ((type.t & VT_BTYPE) == VT_FUNC) {
                    ...
                    if (has_init && (type.t & VT_VLA))
                        nooc_error("variable length array cannot be initialized");
		        ...
		        || (type.t & VT_BTYPE) == VT_FUNC
                        ...
                        if (ad.alias_target) {
                            ...
                            if (!esym)
                                nooc_error("unsupported forward __alias__ attribute");
