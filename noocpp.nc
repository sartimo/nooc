...
static const char nooc_keywords[] = 
...
{
    if (tok != c)
        nooc_error("'%c' expected (got \"%s\")", c, get_tok_str(tok, &tokc));
    next();
}

ST_FUNC void expect(const char *msg)
{
    nooc_error("%s expected", msg);
}
...
#ifndef USE_TAL
#define tal_free(al, p) nooc_free(p)
#define tal_realloc(al, p, size) nooc_realloc(p, size)
...
{
    TinyAlloc *al = nooc_mallocz(sizeof(TinyAlloc));
    al->p = al->buffer = nooc_malloc(size);
    al->limit = limit;
    al->size = size;
    if (pal) *pal = al;
    return al;
...
static void cstr_realloc(CString *cstr, int new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    cstr->data = nooc_realloc(cstr->data, size);
...
ST_INLN char *unicode_to_utf8 (char *b, uint32_t Uc)
{
    ...
    else error: nooc_error("0x%x is not a valid universal character", Uc);
    return b;
}
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if (tok_ident >= SYM_FIRST_ANOM) 
        nooc_error("memory full (symbols)");

    /* expand token table if needed */
    i = tok_ident - TOK_IDENT;
    if ((i % TOK_ALLOC_INCR) == 0) {
        ptable = nooc_realloc(table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym *));
        table_ident = ptable;
    }
...
static uint8_t *parse_comment(uint8_t *p)
{
    ...
    for(;;) {
        ...
        check_eof:
            if (c == CH_EOF)
                nooc_error("unexpected end of file in comment");
            if (c != '\\')
                goto redo;
        }
    }
...
static uint8_t *parse_pp_string(uint8_t *p, int sep, CString *str)
{
    ...
    redo:
        ...
        unterminated_string:
                ...
                nooc_error("missing terminating %c character", sep);
            } else if (c == '\\') {
                if (str)
                    cstr_ccat(str, c);
                c = *++p;
        ...
        add_lf:
            if (ACCEPT_LF_IN_STRINGS) {
                ...
                //nooc_warning("missing terminating %c character", sep);
                return p;
            }
        } else if (c == '\r') {
            c = *++p;
...
static void tok_str_add2(TokenString *s, int t, CValue *cv)
{
    ...
    case TOK_CULONG:
        ...
        break;
    case TOK_CLDOUBLE:
#if LDOUBLE_SIZE == 8 || defined NOOC_USING_DOUBLE_FOR_LDOUBLE
        str[len++] = cv->tab[0];
        str[len++] = cv->tab[1];
...
ST_INLN void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
	...
	nooc_warning("%s redefined", get_tok_str(v, NULL));
}
...
/* fake the nth "#if defined test_..." for nooc -dt -run */
static void maybe_run_test(NOOCState *s)
{
    const char *p;
...
search_cached_include(NOOCState *s1, const char *filename, int add);

static int parse_include(NOOCState *s1, int do_next, int test)
{
    int c, i;
	...
	parse_flags = PARSE_FLAG_PREPROCESS
        ...
        for (;;) {
            ...
            if (i > 0
                ...
                break;
            if (tok == TOK_LINEFEED)
                nooc_error("'#include' expects \"FILENAME\" or <FILENAME>");
    ...
    for (;;) {
        ...
        if (i == 0) {
            ...
            pstrncpy(buf, p, nooc_basename(p) - p);
        } else {
            ...
            if (k < 0)
                ...
                nooc_error("include file '%s' not found", name);
    ...
    if (test) {
        ...
        if (s1->include_stack_ptr >= s1->include_stack + INCLUDE_STACK_SIZE)
            nooc_error("#include recursion too deep");
...
static int expr_preprocess(NOOCState *s1)
{
    int c, t;
    TokenString *str;
      ...
      redo:
        if (tok < TOK_IDENT) {
            if (tok >= TOK_STR && tok <= TOK_CLDOUBLE)
                nooc_error("invalid constant in preprocessor expression");
		...
		expect("(");
            ...
            next();
            if (tok == '(')
                nooc_error("function-like macro '%s' is not defined",
                          get_tok_str(t, NULL));
            goto redo;
        }
        tok_str_add_tok(str);
...
ST_FUNC void parse_define(void)
{
    ...
    if (v < TOK_IDENT || v == TOK_DEFINED)
        nooc_error("invalid macro name '%s'", get_tok_str(tok, &tokc));
    ...
    if (tok == '(') {
        ...
        if (tok != ')') for (;;) {
            ...
            if (varg < TOK_IDENT)
        bad_list:
                nooc_error("bad macro parameter list");
    ...
    if (3 == spc)
bad_twosharp:
        nooc_error("'##' cannot appear at either end of macro");
    define_push(v, t, tok_str_dup(&tokstr_buf), first);
}

static CachedInclude *search_cached_include(NOOCState *s1, const char *filename, int add)
{
    ...
    s = basename = nooc_basename(filename);
    h = TOK_HASH_INIT;
    ...
    for(;;) {
        ...
        if (0 == PATHCMP(filename, e->filename))
            return e;
        if (e->once
            && 0 == PATHCMP(basename, nooc_basename(e->filename))
    ...
    e = nooc_malloc(sizeof(CachedInclude) + (len = strlen(filename)));
...
static void pragma_parse(NOOCState *s1)
{
    next_nomacro();
    if (tok == TOK_push_macro || tok == TOK_pop_macro) {
        ...
        if (s)
            ...
            nooc_warning("unbalanced #pragma pop_macro");
        pp_debug_tok = t, pp_debug_symv = v;

    } else if (tok == TOK_once) {
        search_cached_include(s1, file->filename, 1)->once = 1;

    } else if (s1->output_type == NOOC_OUTPUT_PREPROCESS) {
        /* nooc -E: keep pragmas below unchanged */
        unget_tok(' ');
        ...
        if (tok != TOK_STR)
            goto pragma_err;
        p = nooc_strdup((char *)tokc.str.data);
        next();
        if (tok != ')')
            goto pragma_err;
        if (t == TOK_lib) {
            ...
            if (t == TOK_option)
                nooc_set_options(s1, p);
            nooc_free(p);
        }

    } else
        nooc_warning_c(warn_unsupported)("#pragma %s ignored", get_tok_str(tok, &tokc));
    return;

pragma_err:
    nooc_error("malformed #pragma directive");
    return;
}
...
ST_FUNC void preprocess(int is_bof)
{
 ...
 redo:
    ...
    do_ifdef:
        next_nomacro();
        if (tok < TOK_IDENT)
            nooc_error("invalid argument for '#if%sdef'", c ? "n" : "");
        if (is_bof) {
            if (c) {
#ifdef INC_DEBUG
    ...
    do_if:
        if (s1->ifdef_stack_ptr >= s1->ifdef_stack + IFDEF_STACK_SIZE)
            nooc_error("memory full (ifdef)");
        *s1->ifdef_stack_ptr++ = c;
        goto test_skip;
    case TOK_ELSE:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            nooc_error("#else without matching #if");
        if (s1->ifdef_stack_ptr[-1] & 2)
            nooc_error("#else after #else");
        c = (s1->ifdef_stack_ptr[-1] ^= 3);
        goto test_else;
    case TOK_ELIF:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            nooc_error("#elif without matching #if");
        c = s1->ifdef_stack_ptr[-1];
        if (c > 1)
            nooc_error("#elif after #else");
    ...
    case TOK_ENDIF:
        if (s1->ifdef_stack_ptr <= file->ifdef_stack_ptr)
            nooc_error("#endif without matching #if");
        s1->ifdef_stack_ptr--;
        ...
        if (tok != TOK_LINEFEED) {
            if (tok == TOK_STR) {
                if (file->true_filename == file->filename)
                    file->true_filename = nooc_strdup(file->filename);
    ...
    case TOK_WARNING:
        ...
        *q = '\0';
        if (tok == TOK_ERROR)
            nooc_error("#error %s", buf);
        else
            nooc_warning("#warning %s", buf);
        break;
    case TOK_PRAGMA:
        pragma_parse(s1);
        break;
    ...
    default:
        ...
        if (tok == '!' && is_bof)
            ...
            goto ignore;
        nooc_warning("Ignoring unknown preprocessing directive #%s", get_tok_str(tok, &tokc));
    ignore:
...
static void parse_escape_string(CString *outstr, const uint8_t *buf, int is_long)
{
    ...
    add_hex_or_ucn:
            ...
            invalid_escape:
                if (c >= '!' && c <= '~')
                    nooc_warning("unknown escape sequence: \'\\%c\'", c);
                else
                    nooc_warning("unknown escape sequence: \'\\x%x\'", c);
                break;
            }
        ...
        invalid_utf8_sequence:
            nooc_warning("ill-formed UTF-8 subsequence starting with: \'\\x%x\'", c);
            c = 0xFFFD;
            p += skip;
            goto add_char_nonext;
...
static void parse_string(const char *s, int len)
{
    ...
    if (sep == '\'') {
        ...
        if (n < 1)
            nooc_error("empty character constant");
        if (n > 1)
            nooc_warning_c(warn_all)("multi-character character constant");
        for (c = i = 0; i < n; ++i) {
            if (is_long)
    ...
    *q++ = t;
    ...
    if (t == '.') {
        ...
        if (ch == 'x' || ch == 'X') {
            ...
            ch = *p++;
            b = 16;
        } else if (nooc_state->nooc_ext && (ch == 'b' || ch == 'B')) {
            q--;
            ch = *p++;
            b = 2;
        }
    ...
    if (ch == '.' ||
        ...
        *q = '\0';
        ...
        for(;;) {
            ...
            if (t == 'L') {
                if (lcount >= 2)
                    nooc_error("three 'l's in integer constant");
                if (lcount && *(p - 1) != ch)
                    nooc_error("incorrect integer suffix: %s", p1);
                lcount++;
                ch = *p++;
            } else if (t == 'U') {
                if (ucount >= 1)
                    nooc_error("two 'u's in integer constant");
                ucount++;
                ch = *p++;
            } else {
                break;
        ...
        if (ov)
            nooc_warning("integer constant overflow");
...
maybe_newline:
    ...
    default:
        ...
        if (parse_flags & PARSE_FLAG_ASM_FILE)
            goto parse_simple;
        nooc_error("unrecognized character \\x%02x", c);
        break;
    }
    tok_flags = 0;
keep_tok_flags:
...
static int paste_tokens(int t1, CValue *v1, int t2, CValue *v2)
{
    ...
    nooc_open_bf(nooc_state, ":paste:", tokcstr.size);
    ...
    for (;;) {
        ...
        if (is_space(tok))
            continue;
        nooc_warning("pasting \"%.*s\" and \"%s\" does not give a valid"
    ...
    Sym *s)
{
    ...
    empty_arg:
                ...
                if (!args && !sa && tok == ')')
                    break;
                if (!sa)
                    nooc_error("macro '%s' used with too many args",
                          get_tok_str(s->v, 0));
                tok_str_new(&str);
                parlevel = spc = 0;
            ...
            if (sa) {
                nooc_error("macro '%s' used with too few args",
                      get_tok_str(s->v, 0));
            }
...
static void nooc_predefs(NOOCState *s1, CString *cs, int is_asm)
{
    int a, b, c;

    sscanf(NOOC_VERSION, "%d.%d.%d", &a, &b, &c);
    ...
    if (s1->output_type == NOOC_OUTPUT_PREPROCESS)
      putdef(cs, "__NOOC_PP__");
    if (s1->output_type == NOOC_OUTPUT_MEMORY)
      putdef(cs, "__NOOC_RUN__");
#ifdef CONFIG_NOOC_BACKTRACE
    if (s1->do_backtrace)
      putdef(cs, "__NOOC_BACKTRACE__");
#endif
#ifdef CONFIG_NOOC_BCHECK
    if (s1->do_bounds_check)
      putdef(cs, "__NOOC_BCHECK__");
#endif
...
#if CONFIG_NOOC_PREDEFS
        #include "noocdefs_.h" /* include as strings */
#else
        "#include <noocdefs.h>\n" /* load at runtime */
#endif
        , -1);
...
ST_FUNC void preprocess_start(NOOCState *s1, int filetype)
{
    ...
    if (!(filetype & AFF_TYPE_ASM)) {
        ...
        *s1->include_stack_ptr++ = file;
        nooc_open_bf(s1, "<command line>", cstr.size);
...
static void pp_line(NOOCState *s1, BufferedFile *f, int level)
{
...
static void define_print(NOOCState *s1, int v)
{
    FILE *fp;
    Sym *s;
...
static void pp_debug_defines(NOOCState *s1)
{
    int v, t;
    const char *vs;
    FILE *fp;
...
static void pp_debug_builtins(NOOCState *s1)
{
    int v;
