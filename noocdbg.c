...
/* ------------------------------------------------------------------------- */
static void put_stabs(NOOCState *s1, const char *str, int type, int other,
    int desc, unsigned long value);

ST_FUNC void nooc_debug_new(NOOCState *s1)
{
    int shf = 0;
    if (!s1->dState)
        s1->dState = nooc_mallocz(sizeof *s1->dState);
#ifdef CONFIG_NOOC_BACKTRACE
    /* include stab info with standalone backtrace support */
    if (s1->do_backtrace
        && (s1->output_type & (NOOC_OUTPUT_EXE | NOOC_OUTPUT_DLL)))
...
static void put_stabs(NOOCState *s1, const char *str, int type, int other, int desc,
                      unsigned long value)
{
    Stab_Sym *sym;
...
static void put_stabs_r(NOOCState *s1, const char *str, int type, int other, int desc,
...
static void put_stabn(NOOCState *s1, int type, int other, int desc, int value)
{
	...
	*filename++ = '\0';
	...
	if (i == dwarf_line.dir_size) {
	    ...
	    dwarf_line.dir_table = 
                (char **) nooc_realloc(dwarf_line.dir_table,
                                      dwarf_line.dir_size *
                                      sizeof (char *));
            dwarf_line.dir_table[i] = nooc_strdup(dir);
	}
	*undo = '/';
    }
    dwarf_line.filename_table =
        (struct dwarf_filename_struct *)
        nooc_realloc(dwarf_line.filename_table,
...
static void dwarf_uleb128_op (NOOCState *s1, unsigned long long value)
{
    do {
        unsigned char byte = value & 0x7f;
...
static void dwarf_sleb128_op (NOOCState *s1, long long value)
{
    int more;
    long long end = value >> 63;
    unsigned char last = end & 0x40;
...
ST_FUNC void nooc_debug_start(NOOCState *s1)
{
    ...
    if (s1->do_debug) {
	    ...
	    char *undo;
            ...
            dwarf_strp(dwarf_info_section, "nooc " NOOC_VERSION);
		...
		*undo = 0;
            ...
            dwarf_line.dir_table = (char **) nooc_malloc(sizeof (char *) *
							dwarf_line.dir_size);
            dwarf_line.dir_table[0] = nooc_strdup(buf);
	    if (undo)
                dwarf_line.dir_table[1] = nooc_strdup(filename);
            dwarf_line.filename_size = 2;
            dwarf_line.filename_table =
    	        (struct dwarf_filename_struct *)
    	        nooc_malloc(2*sizeof (struct dwarf_filename_struct));
            dwarf_line.filename_table[0].dir_entry = 0;
	    if (undo) {
                dwarf_line.filename_table[0].name = nooc_strdup(undo + 1);
                dwarf_line.filename_table[1].dir_entry = 1;
                dwarf_line.filename_table[1].name = nooc_strdup(undo + 1);
		*undo = '/';
	    }
	    else {
                dwarf_line.filename_table[0].name = nooc_strdup(filename);
                dwarf_line.filename_table[1].dir_entry = 0;
                dwarf_line.filename_table[1].name = nooc_strdup(filename);
	    }
...
ST_FUNC void nooc_debug_end(NOOCState *s1)
{
    ...
    if (debug_info_root)
        nooc_debug_funcend(s1, 0); /* free stuff in case of errors */
	...
	for (i = 0; i < n_debug_anon_hash; i++) {
	    ...
	    nooc_free (debug_anon_hash[i].debug_type);
	}
	nooc_free (debug_anon_hash);
	...
	for (i = 0; i < dwarf_line.dir_size; i++)
	    nooc_free(dwarf_line.dir_table[i]);
	nooc_free(dwarf_line.dir_table);
	for (i = 0; i < dwarf_line.filename_size; i++)
	    nooc_free(dwarf_line.filename_table[i].name);
	nooc_free(dwarf_line.filename_table);
	...
	nooc_free(dwarf_line.line_data);
    ...
    nooc_free(debug_hash);
    debug_next_type = 0;
}

static BufferedFile* put_new_file(NOOCState *s1)
{
    BufferedFile *f = file;
...
static void nooc_debug_stabs (NOOCState *s1, const char *str, int type, unsigned long value,
                             Section *sec, int sym_index, int info)
{
    ...
    if (debug_info) {
        debug_info->sym =
            (struct debug_sym *)nooc_realloc (debug_info->sym,
                                             sizeof(struct debug_sym) *
                                             (debug_info->n_sym + 1));
        s = debug_info->sym + debug_info->n_sym++;
        s->type = type;
        s->value = value;
        s->str = nooc_strdup(str);
        s->sec = sec;
...
ST_FUNC void nooc_debug_stabn(NOOCState *s1, int type, int value)
{
    ...
    if (type == N_LBRAC) {
        struct _debug_info *info =
            (struct _debug_info *) nooc_mallocz(sizeof (*info));
...
static int nooc_debug_find(NOOCState *s1, Sym *t, int dwarf)
{
    int i;
...
static int nooc_get_dwarf_info(NOOCState *s1, Sym *s);

static void nooc_debug_check_anon(NOOCState *s1, Sym *t, int debug_type)
{
    int i;
	...
	for (i = 0; i < n_debug_anon_hash; i++)
		...
		debug_anon_hash[i].debug_type =
		    nooc_realloc(debug_anon_hash[i].debug_type,
...
ST_FUNC void nooc_debug_fix_anon(NOOCState *s1, CType *t)
{
	...
	for (i = 0; i < n_debug_anon_hash; i++)
		...
		debug_type = nooc_get_dwarf_info(s1, &sym);
		debug_info = NULL;
		for (j = 0; j < debug_anon_hash[i].n_debug_type; j++)
		    write32le(dwarf_info_section->data +
			      debug_anon_hash[i].debug_type[j],
...
static void nooc_debug_remove(NOOCState *s1, Sym *t)
{
    int i;
...
static void nooc_get_debug_info(NOOCState *s1, Sym *s, CString *result)
{
    int type;
    int n = 0;
    int debug_type = -1;
	...
	Sym *e = t;
        ...
        t = t->type.ref;
	debug_type = nooc_debug_find(s1, t, 0);
        if (debug_type == -1) {
            debug_type = nooc_debug_add(s1, t, 0);
            cstr_new (&str);
		...
		if (STRUCT_NODEBUG(t))
            ...
            cstr_printf (&str, ";");
            nooc_debug_stabs(s1, str.data, N_LSYM, 0, NULL, 0, 0);
            cstr_free (&str);
            if (debug_info)
                nooc_debug_remove(s1, e);
        }
    }
    ...
    for (;;) {
        ...
        if (type == VT_PTR)
            ...
            nooc_get_debug_info (s1, t->type.ref, result);
            return;
        }
        else
            break;
...
static int nooc_get_dwarf_info(NOOCState *s1, Sym *s)
{
    ...
    if ((type & VT_BTYPE) == VT_STRUCT) {
        t = t->type.ref;
	debug_type = nooc_debug_find(s1, t, 1);
	if (debug_type == -1) {
	    int pos_sib = 0, i, *pos_type;

	    debug_type = nooc_debug_add(s1, t, 1);
	    e = t;
	    i = 0;
	    while (e->next) {
		e = e->next;
	    ...
	    pos_type = (int *) nooc_malloc(i * sizeof(int));
	    dwarf_data1(dwarf_info_section,
			IS_UNION (t->type.t)
		...
		if (STRUCT_NODEBUG(e))
		    continue;
		type = nooc_get_dwarf_info(s1, e);
		nooc_debug_check_anon(s1, e, pos_type[i]);
		write32le(dwarf_info_section->data + pos_type[i++],
			  type - dwarf_info.start);
	    }
	    nooc_free(pos_type);
	    if (debug_info)
		nooc_debug_remove(s1, t);
        }
    }
    else if (IS_ENUM(type)) {
        t = t->type.ref;
	...
	if (debug_type == -1) {
	    ...
	    pos_type = nooc_get_dwarf_info(s1, &sym);
	    debug_type = nooc_debug_add(s1, t, 1);
    ...
    for (;;) {
	    ...
	    dwarf_data1(dwarf_info_section, PTR_SIZE);
	    if (last_pos != -1) {
		nooc_debug_check_anon(s1, e, last_pos);
	    ...
	    sub_type = nooc_get_dwarf_info(s1, &sym);
	    i = dwarf_info_section->data_offset;
	    if (retval == debug_type)
		retval = i;
	    ...
	    i = dwarf_info_section->data_offset;
	    debug_type = nooc_get_dwarf_info(s1, t->type.ref);
	    if (retval == debug_type)
		retval = i;
	    dwarf_data1(dwarf_info_section,
	    ...
	    while (f->next) {
		f = f->next;
		type = nooc_get_dwarf_info(s1, f);
		nooc_debug_check_anon(s1, f, pos_type[i]);
	        write32le(dwarf_info_section->data + pos_type[i++],
                          type - dwarf_info.start);
	    }
	    nooc_free(pos_type);
...
static void nooc_debug_finish (NOOCState *s1, struct _debug_info *cur)
{
    while (cur) {
...
#endif
            nooc_debug_finish (s1, cur->child);
	    if (cur->child)
                dwarf_data1(dwarf_info_section, 0);
        }
        else
...
ST_FUNC void nooc_add_debug_info(NOOCState *s1, int param, Sym *s, Sym *e)
{
    ...
    for (; s != e; s = s->prev) {
        if (!s->v || (s->r & VT_VALMASK) != VT_LOCAL)
            continue;
	if (s1->dwarf) {
	    nooc_debug_stabs(s1, get_tok_str(s->v, NULL),
			    param ? N_PSYM : N_LSYM, s->c, NULL, 0,
			    nooc_get_dwarf_info(s1, s));
	}
	else
        {
            ...
            cstr_printf (&debug_str, "%s:%s", get_tok_str(s->v, NULL),
			 param ? "p" : "");
            nooc_get_debug_info(s1, s, &debug_str);
            nooc_debug_stabs(s1, debug_str.data, param ? N_PSYM : N_LSYM,
			    s->c, NULL, 0, 0);
	}
    }
    cstr_free (&debug_str);
...
ST_FUNC void nooc_debug_funcstart(NOOCState *s1, Sym *sym)
{
    ...
    debug_info = NULL;
    nooc_debug_stabn(s1, N_LBRAC, ind - func_ind);
    f = put_new_file(s1);
    if (!f)
	return;
    ...
    else
    {
        ...
        nooc_get_debug_info(s1, sym->type.ref, &debug_str);
...
ST_FUNC void nooc_debug_funcend(NOOCState *s1, int size)
{
    ...
    nooc_debug_line(s1);
    ind += min_instr_len;
    nooc_debug_stabn(s1, N_RBRAC, size);
    if (s1->dwarf) {
        int func_sib = 0;
	Sym *sym = dwarf_info.func;
	int n_debug_info = nooc_get_dwarf_info(s1, sym->type.ref);
	...
	nooc_debug_check_anon(s1, sym->type.ref, dwarf_info_section->data_offset);
...
#endif
        nooc_debug_finish (s1, debug_info_root);
	dwarf_data1(dwarf_info_section, 0);
...
ST_FUNC void nooc_debug_extern_sym(NOOCState *s1, Sym *sym, int sh_num, int sym_bind, int sym_type)
{
	...
	dwarf_uleb128(dwarf_info_section, dwarf_line.cur_file);
	dwarf_uleb128(dwarf_info_section, file->line_num);
	nooc_debug_check_anon(s1, sym, dwarf_info_section->data_offset);
    ...
    else
    {
        ...
        nooc_get_debug_info(s1, sym, &str);
        if (sym_bind == STB_GLOBAL)
            nooc_debug_stabs(s1, str.data, N_GSYM, 0, NULL, 0, 0);
        else
            nooc_debug_stabs(s1, str.data,
...
ST_FUNC void nooc_tcov_block_begin(NOOCState *s1)
{
    ...
    nooc_tcov_block_end (nooc_state, 0);
...
ST_FUNC void nooc_tcov_check_line(NOOCState *s1, int start)
{
    ...
    if (tcov_data.line != file->line_num) {
	    ...
	    nooc_tcov_block_end (s1, -1);
	    if (start)
                nooc_tcov_block_begin (s1);
	}
	else
	    tcov_data.line = file->line_num;
    }
...
ST_FUNC void nooc_tcov_start(NOOCState *s1)
{
    ...
    if (tcov_section == NULL) {
        tcov_section = new_section(nooc_state, ".tcov", SHT_PROGBITS,
				   SHF_ALLOC | SHF_WRITE);
