...
#if !defined NOOC_TARGET_X86_64 && !defined NOOC_TARGET_ARM64
#error Platform not supported
#endif

/* XXX: this file uses nooc_error() to the effect of exit(1) */
#undef _nooc_error
...
{
    struct load_command *lc = nooc_mallocz(cmdsize);
    lc->cmd = cmd;
    lc->cmdsize = cmdsize;
    mo->lc = nooc_realloc(mo->lc, sizeof(mo->lc[0]) * (mo->nlc + 1));
    mo->lc[mo->nlc++] = lc;
    return lc;
}
...
static struct segment_command_64 * add_segment(struct macho *mo, const char *name)
{
    ...
    mo->seg2lc = nooc_realloc(mo->seg2lc, sizeof(*mo->seg2lc) * (mo->nseg + 1));
...
static int add_section(struct macho *mo, struct segment_command_64 **_seg, const char *name)
{
    ...
    seg = nooc_realloc(seg, sizeof(*seg) + seg->nsects * sizeof(*sec));
...
static void nooc_macho_add_destructor(NOOCState *s1)
{
...
{
    mo->bind_rebase = nooc_realloc(mo->bind_rebase, (mo->n_bind_rebase + 1) *
...
static void check_relocs(NOOCState *s1, struct macho *mo)
{
    Section *s;
    ...
    for (i = 1; i < s1->nb_sections; i++) {
	    ...
	    save_rel = *rel;
            ...
            if (sym->st_shndx == SHN_UNDEF
	 	    ...
	 	    goti = nooc_realloc(goti, (mo->n_got + 1) * sizeof(*goti));
                    if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
                        if (sym->st_shndx == SHN_UNDEF)
                          nooc_error("undefined local symbo: '%s'",
    ...
    for (i = 1; i < s1->nb_sections; i++) {
...
#endif
                        mo->s_lazy_bind =
                            nooc_realloc(mo->s_lazy_bind, (mo->n_lazy_bind + 1) *
		...
		if (sym->st_shndx == SHN_UNDEF) {
		    ...
		    mo->n_bind++;
                    ...
                    mo->s_rebase =
                        nooc_realloc(mo->s_rebase, (mo->n_rebase + 1) *
...
static int check_symbols(NOOCState *s1, struct macho *mo)
{
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (bind == STB_LOCAL) {
            ...
            if (mo->iextdef != -1 || mo->iundef != -1)
              nooc_error("local syms after global ones");
        } else if (sym->st_shndx != SHN_UNDEF) {
            if (mo->iextdef == -1)
              mo->iextdef = sym_index - 1;
            if (mo->iundef != -1)
              nooc_error("external defined symbol after undefined");
        } else if (sym->st_shndx == SHN_UNDEF) {
            if (mo->iundef == -1)
...
static void convert_symbol(NOOCState *s1, struct macho *mo, struct nlist_64 *pn)
{
    ...
    case STT_FILE:
        ...
        break;
    default:
        nooc_error("unhandled ELF symbol type %d %s",
                  ELFW(ST_TYPE)(sym->st_info), name);
    }
    if (sym->st_shndx == SHN_UNDEF)
      nooc_error("should have been rewritten to SHN_FROMDLL: %s", name);
    else if (sym->st_shndx == SHN_FROMDLL)
      n.n_type = N_UNDF, n.n_sect = 0;
    else if (sym->st_shndx == SHN_ABS)
      n.n_type = N_ABS, n.n_sect = 0;
    else if (sym->st_shndx >= SHN_LORESERVE)
      nooc_error("unhandled ELF symbol section %d %s", sym->st_shndx, name);
    else if (!mo->elfsectomacho[sym->st_shndx]) {
      if (strncmp(s1->sections[sym->st_shndx]->name, ".debug_", 7))
        nooc_error("ELF section %d(%s) not mapped into Mach-O for symbol %s",
...
static void convert_symbols(NOOCState *s1, struct macho *mo)
{
    struct nlist_64 *pn;
...
static void nooc_qsort (void  *base, size_t nel, size_t width,
...
static void create_symtab(NOOCState *s1, struct macho *mo)
{
    int sym_index, sym_end;
    struct nlist_64 *pn;
    ...
    nooc_qsort(pn, sym_end - 1, sizeof(*pn), machosymcmp, s1);
    mo->e2msym = nooc_malloc(sym_end * sizeof(*mo->e2msym));
    mo->e2msym[0] = -1;
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
...
#ifdef CONFIG_NEW_MACHO
static void calc_fixup_size(NOOCState *s1, struct macho *mo)
{
    int i, size;
    ...
    for (i = (s1->output_type == NOOC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
...
static void set_segment_and_offset(NOOCState *s1, struct macho *mo, addr_t addr,
...
static void bind_rebase(NOOCState *s1, struct macho *mo)
{
    int i;
    uint8_t *ptr;
    ElfW(Sym) *sym;
	...
	*ptr = REBASE_OPCODE_DO_REBASE_IMM_TIMES | 1;
    ...
    nooc_free(mo->s_lazy_bind);
    nooc_free(mo->s_rebase);
    nooc_free(mo->bind);
}
#endif
                        ...
                        int n_trie, struct trie_info *trie)
{
    ...
    for (i = from; i < to; i = end) {
        ...
        node->child = nooc_realloc(node->child,
                                  (node->n_child + 1) *
                        ...
                        int n_trie, struct trie_info *trie)
{
    ...
    for (i = 0; i < node->n_child; i++) {
        p_nest = &node->child[i];
        *seq = nooc_realloc(*seq, (*n_seq + 1) * sizeof(struct trie_seq));
        p_seq = &(*seq)[(*n_seq)++];
...
static void export_trie(NOOCState *s1, struct macho *mo)
{
    ...
    addr_t vm_addr = get_segment(mo, s1->output_type == NOOC_OUTPUT_EXE)->vmaddr;
	...
	if (sym->st_shndx != SHN_UNDEF && sym->st_shndx < SHN_LORESERVE &&
	    ...
	    trie = nooc_realloc(trie, (n_trie + 1) * sizeof(struct trie_info));
    ...
    if (n_trie) {
        nooc_qsort(trie, n_trie, sizeof(struct trie_info), triecmp, NULL);
...
static void collect_sections(NOOCState *s1, struct macho *mo, const char *filename)
{
    int i, sk, numsec;
	...
	if (used_segment[i]) {
	    ...
	    if (i == 1 && s1->output_type != NOOC_OUTPUT_EXE)
	        seg->vmaddr = 0;
	    else
    ...
    curaddr = get_segment(mo, s1->output_type == NOOC_OUTPUT_EXE)->vmaddr;
    curaddr += 4096;
    seg = NULL;
    numsec = 0;
    mo->elfsectomacho = nooc_mallocz(sizeof(*mo->elfsectomacho) * s1->nb_sections);
    for (sk = sk_unknown; sk < sk_last; sk++) {
	...
	if (sk == sk_linkedit) {
	    ...
	    (s1->output_type != NOOC_OUTPUT_EXE || mo->segment[sk]) &&
	    mo->sk_to_sect[sk].s) {
            uint64_t al = 0;
            int si;
		...
		if (sk == sk_la_ptr)
            ...
            al = 1ULL << al;
            if (al > 4096)
              nooc_warning("alignment > 4096"), sec->align = 12, al = 4096;
...
static void macho_write(NOOCState *s1, struct macho *mo, FILE *fp)
{
    int i, sk;
    uint64_t fileofs = 0;
    Section *s;
...
ST_FUNC void bind_rebase_import(NOOCState *s1, struct macho *mo)
{
    ...
    nooc_qsort(mo->bind_rebase, mo->n_bind_rebase, sizeof(struct bind_rebase),
	      bind_rebase_cmp, s1);
...
#define	PTR_64_MASK   0xFFFFFFFFFFFFFFULL
	...
	for (j = 0; j < page_count; j++) {
		...
		if ((addr & 3) ||
		    ...
		    nooc_error("Illegal rel_offset %s %lld",
...
ST_FUNC int macho_output_file(NOOCState *s1, const char *filename)
{
    ...
    if (fd < 0 || (fp = fdopen(fd, "wb")) == NULL) {
        nooc_error_noabort("could not write '%s: %s'", filename, strerror(errno));
        return -1;
    }
    if (s1->verbose)
        printf("<- %s\n", filename);
 ...
 do_ret:
	...
	if (retval == -1 || !(WIFEXITED(retval) && WEXITSTATUS(retval) == 0))
	    nooc_error ("command failed '%s'", command);
    }
#endif
    return ret;
}
...
#pragma pop_macro("free")
    if (path.size)
        nooc_add_library_path(s, (char*)path.data);
    else
        nooc_add_library_path(s,
...
ST_FUNC const char* macho_tbd_soname(const char* filename) {
    ...
    tbd_parse_trample;
    ret = nooc_strdup(soname);
the_end:
    nooc_free(data);
    return ret;
}
#endif /* NOOC_IS_NATIVE */
...
ST_FUNC int macho_load_tbd(NOOCState* s1, int fd, const char* filename, int lev)
{
    ...
    tbd_parse_trample;
    ret = 0;
    if (nooc_add_dllref(s1, soname, lev)->found)
        goto the_end;
    while(pos) {
        char* sym = NULL;
        int cont = 1;
...
ST_FUNC int macho_load_dll(NOOCState * s1, int fd, const char* filename, int lev)
{
  ...
  again:
    ...
    if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
        ...
        nooc_warning("%s: Mach-O fat 64bit files of type 0x%x not handled",
                    filename, fh.magic);
        return -1;
    }
    ...
    for (i = 0, lc = buf2; i < mh.ncmds; i++) {
        ...
        case LC_REEXPORT_DYLIB:
        {
            ...
            dprintf(" REEXPORT %s\n", name);
            if (subfd < 0)
              nooc_warning("can't open %s (reexported from %s)", name, filename);
            else {
    ...
    if (!nsyms || !nextdef)
      nooc_warning("%s doesn't export any symbols?", filename);
