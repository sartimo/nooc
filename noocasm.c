...
static int asm_get_prefix_name(NOOCState *s1, const char *prefix, unsigned int n)
{
    char buf[64];
...
ST_FUNC int asm_get_local_label_name(NOOCState *s1, unsigned int n)
{
    return asm_get_prefix_name(s1, "L..", n);
}

static int nooc_assemble_internal(NOOCState *s1, int do_preprocess, int global);
static Sym* asm_new_label(NOOCState *s1, int label, int is_local);
static Sym* asm_new_label1(NOOCState *s1, int label, int is_local, int sh_num, int value);
...
static Sym* asm_section_sym(NOOCState *s1, Section *sec)
{
...
static void asm_expr_unary(NOOCState *s1, ExprValue *pe)
{
    Sym *sym;
    int op, label;
    uint64_t n;
    ...
    case TOK_PPNUM:
        ...
        if (*p == 'b' || *p == 'f') {
            ...
            if (*p == 'b') {
                ...
                if (sym && (!sym->c || elfsym(sym)->st_shndx == SHN_UNDEF))
                    sym = sym->prev_tok;
                if (!sym)
                    nooc_error("local label '%d' not found backward", (int)n);
            } else {
                /* forward */
                if (!sym || (sym->c && elfsym(sym)->st_shndx != SHN_UNDEF)) {
	    ...
	    pe->pcrel = 0;
            ...
            nooc_error("invalid number syntax");
        }
        next();
        break;
    case '+':
    ...
    case '~':
        ...
        asm_expr_unary(s1, pe);
        if (pe->sym)
            nooc_error("invalid operation with label");
        if (op == '-')
            pe->v = -pe->v;
        else
            pe->v = ~pe->v;
    ...
    default:
		...
		pe->pcrel = 0;
            ...
            nooc_error("bad expression syntax [%s]", get_tok_str(tok, &tokc));
        }
        break;
    }
}
    
static void asm_expr_prod(NOOCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static void asm_expr_logic(NOOCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static inline void asm_expr_sum(NOOCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static inline void asm_expr_cmp(NOOCState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static Sym* asm_new_label1(NOOCState *s1, int label, int is_local,
	   ...
	   definitions won't have VT_EXTERN set.  */
        if (esym && esym->st_shndx != SHN_UNDEF) {
            ...
            if (!(sym->type.t & VT_EXTERN))
                nooc_error("assembler label '%s' already defined",
...
static Sym* set_symbol(NOOCState *s1, int label)
{
    long n;
    ExprValue e;
    Sym *sym;
...
static void use_section1(NOOCState *s1, Section *sec)
{
...
static void use_section(NOOCState *s1, const char *name)
{
    Section *sec;
...
static void push_section(NOOCState *s1, const char *name)
{
...
static void pop_section(NOOCState *s1)
{
    Section *prev = cur_text_section->prev;
    if (!prev)
        nooc_error(".popsection without .pushsection");
    cur_text_section->prev = NULL;
    use_section1(s1, prev);
}

static void asm_parse_directive(NOOCState *s1, int global)
{
    ...
    case TOK_ASMDIR_space:
        ...
        {
            if (n < 0 || n > 30)
                nooc_error("invalid p2align, must be between 0 and 30");
            n = 1 << n;
    ...
    case TOK_ASMDIR_fill:
        {
            ...
            if (repeat < 0) {
                nooc_error("repeat < 0; .fill ignored");
                break;
            }
            size = 1;
            val = 0;
            if (tok == ',') {
                ...
                if (size < 0) {
                    nooc_error("size < 0; .fill ignored");
                    break;
                }
                if (size > 8)
                    size = 8;
    ...
    case TOK_ASMDIR_rept:
        {
            ...
            while (next(), tok != TOK_ASMDIR_endr) {
                if (tok == CH_EOF)
                    nooc_error("we at end of file, .endr not found");
            ...
            while (repeat-- > 0) {
                nooc_assemble_internal(s1, (parse_flags & PARSE_FLAG_PREPROCESS),
				      global);
    ...
    case TOK_ASMDIR_org:
		...
		n += esym->st_value;
            ...
            if (n < ind)
                nooc_error("attempt to .org backwards");
            v = 0;
            size = n - ind;
            goto zero_pad;
        }
    ...
    case TOK_ASMDIR_file:
        {
            ...
            nooc_warning_c(warn_unsupported)("ignoring .file %s", filename);
            next();
        }
        break;
    case TOK_ASMDIR_ident:
        {
            ...
            nooc_warning_c(warn_unsupported)("ignoring .ident %s", ident);
            next();
        }
        break;
    case TOK_ASMDIR_size:
        { 
            ...
            if (!sym) {
                nooc_error("label not found: %s", get_tok_str(tok, NULL));
            }
            /* XXX .size name,label2-label1 */
            nooc_warning_c(warn_unsupported)("ignoring .size %s,*", get_tok_str(tok, NULL));
            next();
            skip(',');
    ...
    case TOK_ASMDIR_type:
        { 
            ...
            if (!strcmp(newtype, "function") || !strcmp(newtype, "STT_FUNC")) {
                ...
                nooc_warning_c(warn_unsupported)("change type of '%s' from 0x%x to '%s' ignored",
    ...
    case TOK_ASMDIR_previous:
        { 
            ...
            if (!last_text_section)
                nooc_error("no previous section referenced");
            sec = cur_text_section;
...
#endif
    default:
        nooc_error("unknown assembler directive '.%s'", get_tok_str(tok, NULL));
        break;
    }
}
...
ST_FUNC int nooc_assemble(NOOCState *s1, int do_preprocess)
{
    ...
    ind = cur_text_section->data_offset;
    nocode_wanted = 0;
    ret = nooc_assemble_internal(s1, do_preprocess, 1);
    cur_text_section->data_offset = ind;
    nooc_debug_end(s1);
    return ret;
}
...
static void nooc_assemble_inline(NOOCState *s1, char *str, int len, int global)
{
    const int *saved_macro_ptr = macro_ptr;
    int dotid = set_idnum('.', IS_ID);
    int dolid = set_idnum('$', 0);

    nooc_open_bf(s1, ":asm:", len);
    memcpy(file->buffer, str, len);
    macro_ptr = NULL;
    nooc_assemble_internal(s1, 0, global);
    nooc_close();
                               ...
                               CString *out_str, const char *str)
{
    ...
    for(;;) {
		...
		*str == 'P')
            ...
            if (index < 0)
                nooc_error("invalid operand reference after %%");
            op = &operands[index];
            if (modifier == 'l') {
    ...
    if (tok != ':') {
        ...
        for(;;) {
            if (nb_operands >= MAX_ASM_OPERANDS)
                nooc_error("too many asm operands");
...
ST_FUNC void asm_instr(void)
{
            ...
            if (tok != ')') {
                ...
                if (tok == ':') {
                    ...
                    for (;;) {
                        ...
                        next();
                        asmname = asm_get_prefix_name(nooc_state, "LG.",
                                                      ++asmgoto_n);
                        if (!csym->c)
    ...
    sec = cur_text_section;
    /* assemble the string with nooc internal assembler */
    nooc_assemble_inline(nooc_state, astr.data, astr.size - 1, 0);
    cstr_free_s(&astr);
    if (sec != cur_text_section) {
        nooc_warning("inline asm tries to change current section");
        use_section1(nooc_state, sec);
    }
...
ST_FUNC void asm_global_instr(void)
{
    ...
    /* assemble the string with nooc internal assembler */
    nooc_assemble_inline(nooc_state, astr->data, astr->size - 1, 1);
...
ST_FUNC int nooc_assemble(NOOCState *s1, int do_preprocess)
{
    nooc_error("asm not supported");
}

ST_FUNC void asm_instr(void)
{
    nooc_error("inline asm() not supported");
}
