...
static const char *pe_export_name(NOOCState *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(NOOCState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
static void pe_create_pdb(NOOCState *s1, const char *exename)
{
    char buf[300]; int r;
    snprintf(buf, sizeof buf, "cv2pdb.exe %s", exename);
    r = system(buf);
    strcpy(nooc_fileextension(strcpy(buf, exename)), ".pdb");
    if (r) {
        nooc_error_noabort("could not create '%s'\n(need working cv2pdb from https://github.com/rainers/cv2pdb)", buf);
    } else if (s1->verbose) {
        printf("<- %s\n", buf);
    }
}
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    if (NULL == pf.op)
        return nooc_error_noabort("could not write '%s': %s", pe->filename, strerror(errno));
...
static void pe_build_imports(struct pe_info *pe)
{
    ...
    for (i = 0; i < pe->imp_count; ++i) {
        ...
        if (dllindex)
            name = nooc_basename((dllref = pe->s1->loaded_dlls[dllindex-1])->name);
        else
            name = "", dllref = NULL;
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (dllref)
                    ...
                    ordinal = 0, v = imp_sym->st_value; /* address from nooc_add_symbol() */
                ...
                if (pe->type == PE_RUN) {
                    ...
                    if (!v)
                        nooc_error_noabort("could not resolve symbol '%s'", name);
                } else
#endif
                if (ordinal) {
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    pe_align_section(pe->thunk, 16);
    dllname = nooc_basename(pe->filename);
    ...
    strcpy(nooc_fileextension(buf), ".def");
    op = fopen(buf, "wb");
    if (NULL == op) {
        nooc_error_noabort("could not create '%s': %s", buf, strerror(errno));
    } else {
...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    nbs = s1->nb_sections;
    sec_order = nooc_mallocz(2 * sizeof (int) * nbs);
...
static int pe_isafunc(NOOCState *s1, int sym_index)
{
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                /* nooc_realloc might have altered sym's address */
                sym = (ElfW(Sym) *)symtab_section->data + sym_index;
        ...
        not_found:
            if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
                ...
                continue;
            ret = nooc_error_noabort("undefined symbol '%s'%s", name,
...
static void pe_print_sections(NOOCState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
...
found:
    ...
    for (i = 0; i < ied.NumberOfNames; ++i) {
        ...
        for (l = 0;;) {
            if (n+1 >= n0)
                p = nooc_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(NOOCState *s1, int fd)
{
...
static int pe_load_def(NOOCState *s1, int fd)
{
    int state = 0, ret = -1, dllindex = 0, ord;
    char dllname[80], *buf, *line, *p, *x, next;

    buf = nooc_load_text(fd);
    for (line = buf;; ++line)  {
        ...
        case 1:
            ...
            break;
        case 2:
            dllindex = nooc_add_dllref(s1, dllname, 0)->index;
            ++state;
            /* fall through */
        default:
...
static int pe_load_dll(NOOCState *s1, int fd, const char *filename)
{
    char *p, *q;
    int index, ret;
    ...
    if (ret) {
        ...
        index = nooc_add_dllref(s1, filename, 0)->index;
...
ST_FUNC int pe_load_file(struct NOOCState *s1, int fd, const char *filename)
{
    int ret = -1;
    char buf[10];
    if (0 == strcmp(nooc_fileextension(filename), ".def"))
...
/* ------------------------------------------------------------- */
#ifdef NOOC_TARGET_X86_64
static unsigned pe_add_uwwind_info(NOOCState *s1)
{
    if (NULL == s1->uw_pdata) {
...
static void pe_add_runtime(NOOCState *s1, struct pe_info *pe)
{
    const char *start_symbol;
    int pe_type;

    if (NOOC_OUTPUT_DLL == s1->output_type) {
        pe_type = PE_DLL;
        ...
        if (NOOC_OUTPUT_MEMORY == s1->output_type)
    ...
    /* grab the startup code from libnooc1.a */
#ifdef NOOC_IS_NATIVE
    if (NOOC_OUTPUT_MEMORY != s1->output_type || s1->runtime_main)
#endif
    ...
    if (0 == s1->nostdlib) {
        ...
        for (pp = libs; 0 != (p = *pp); ++pp) {
            if (*p)
                nooc_add_library_err(s1, p);
            else if (PE_DLL != pe_type && PE_GUI != pe_type)
                break;
        }
    }

    /* need this for 'noocelf.c:relocate_sections()' */
    if (NOOC_OUTPUT_DLL == s1->output_type)
...
static void pe_set_options(NOOCState * s1, struct pe_info *pe)
{
    if (PE_DLL == pe->type) {
