...
static void set_options(NOOCState *s, int argc, char **argv)
{
    int i;
    for (i = 1; i < argc; ++i) {
        char *a = argv[i];
...
static int run_callback(const char *src, callback_type callback) {
  ...
  if (nooc_set_output_type(s, NOOC_OUTPUT_MEMORY) == -1)
    return -1;
  if (nooc_compile_string(s, src) == -1)
    return -1;
  if (nooc_relocate(s, NOOC_RELOCATE_AUTO) == -1)
    return -1;
  
  ptr = nooc_get_symbol(s, "f");
  if (!ptr)
    return -1;
  result = callback(ptr);
 ...
 * address, thus causing a success message even though NOOC actually
