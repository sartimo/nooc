/*
 *  NOOC - New Object Oriented C - Support for -run switch
 *
...
typedef struct rt_context
{
    ...
    /* --> noocelf.c:nooc_add_btstub wants those below in that order: */
    union {
	struct {
...
#endif /* CONFIG_NOOC_BACKTRACE */

/* defined when included from lib/bt-exe.c */
#ifndef CONFIG_NOOC_BACKTRACE_ONLY

#ifndef _WIN32
# include <sys/mman.h>
#endif

static int set_pages_executable(NOOCState *s1, int mode, void *ptr, unsigned long length);
static int nooc_relocate_ex(NOOCState *s1, void *ptr, addr_t ptr_diff);

#ifdef _WIN64
static void *win64_add_function_table(NOOCState *s1);
...
LIBNOOCAPI int nooc_relocate(NOOCState *s1, void *ptr)
{
    ...
    if (NOOC_RELOCATE_AUTO != ptr)
        return nooc_relocate_ex(s1, ptr, 0);

    size = nooc_relocate_ex(s1, NULL, 0);
    if (size < 0)
        return -1;
...
#ifdef HAVE_SELINUX
{
    ...
    void *prx;
    char tmpfname[] = "/tmp/.noocrunXXXXXX";
	...
	return nooc_error_noabort("noocrun: could not map memory");
...
#endif
    if (nooc_relocate_ex(s1, ptr, ptr_diff))
        return -1;
...
ST_FUNC void nooc_run_free(NOOCState *s1)
{
    ...
    nooc_free(s1->runtime_mem);
}

static void run_cdtors(NOOCState *s1, const char *start, const char *end,
...
LIBNOOCAPI int nooc_run(NOOCState *s1, int argc, char **argv)
{
    ...
    nooc_add_symbol(s1, "exit", rt_exit);
    nooc_add_symbol(s1, "atexit", rt_atexit);
    nooc_add_symbol(s1, "on_exit", rt_on_exit);
    if (nooc_relocate(s1, NOOC_RELOCATE_AUTO) < 0)
        return -1;
...
#endif
#endif
        rc->top_func = nooc_get_symbol(s1, "main");
        rc->num_callers = s1->rt_num_callers;
        if ((p = nooc_get_symbol(s1, "__rt_error")))
            *(void**)p = _rt_error;
#ifdef CONFIG_NOOC_BCHECK
        if (s1->do_bounds_check) {
            rc->bounds_start = (void*)bounds_section->sh_addr;
            if ((p = nooc_get_symbol(s1, "__bound_init")))
    ...
    run_on_exit(ret);
    if (s1->dflag & 16 && ret) /* nooc -dt -run ... */
...
#elif defined NOOC_TARGET_I386 || defined NOOC_TARGET_X86_64
...
static int set_pages_executable(NOOCState *s1, int mode, void *ptr, unsigned long length)
{
    ...
    if (mprotect((void *)start, end - start, protect[mode]))
        return nooc_error_noabort("mprotect failed: did you mean to configure --with-selinux?");
/* XXX: BSD sometimes dump core with bad system call */
# if (defined NOOC_TARGET_ARM && !TARGETOS_BSD) || defined NOOC_TARGET_ARM64
    if (mode == 0 || mode == 3) {
...
#endif
#endif //ndef CONFIG_NOOC_BACKTRACE_ONLY
/* ------------------------------------------------------------- */
#ifdef CONFIG_NOOC_BACKTRACE
...
static int _rt_error(void *fp, void *ip, const char *fmt, va_list ap)
{
    ...
    if (fp) {
        /* we're called from nooc_backtrace. */
        rc->fp = (addr_t)fp;
        rc->ip = (addr_t)ip;
        msg = "";
    } else {
...
static void set_exception_handler(void)
{
    ...
    /* install NOOC signal handlers to print debug info on fatal
       runtime errors */
...
static NOOCSyms nooc_syms[] = {
#if !defined(CONFIG_NOOCBOOT)
#define NOOCSYM(a) { #a, &a, },
    NOOCSYM(printf)
    NOOCSYM(fprintf)
    NOOCSYM(fopen)
    NOOCSYM(fclose)
